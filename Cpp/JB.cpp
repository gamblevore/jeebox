...
Message* JB_API__errors() {
	...
	Message* result = (JB_Syntax_Msg$str$str(JB_SyxList, JB_str_0));
	{
		...
		while (err) {
			...
			Message* node = (JB_Syntax_Msg$str$str(JB_SyxStr, JB_Err_Render(err, nil)));
...
int JB_Constants__Init_() {
	{
		...
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_228, JB_str_52));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_44, JB_str_83));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_229, JB_str_230));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_16, JB_str_231));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_51, JB_str_232));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_22, JB_str_233));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_54, JB_str_234));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_235, JB_str_236));
		...
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_229, JB_str_230));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_16, JB_str_231));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_51, JB_str_232));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_54, JB_str_234));
		...
		JB_SetRef(JB__Constants_CSWordMiddle, JB_Str_Charset(JB_str_241, true));
		JB_SetRef(JB__Constants_CSWordStart, JB_Str_Charset(JB_str_242, true));
		JB_SetRef(JB__Constants_CSNum, JB__Constants_CSWordMiddle);
		JB_SetRef(JB__Constants_CSLine, JB_Str_Charset(JB_str_243, false));
		JB_SetRef(JB__Constants_CSAfterTemporal, JB_Str_Charset(JB_str_244, false));
		JB_SetRef(JB__Constants_CSLineBlack, JB_Str_Charset(JB_str_245, false));
		JB_SetRef(JB__Constants_CSLettersOnly, JB_Str_Charset(JB_str_246, true));
...
JB_String* JB_Constants__Test() {
	...
	JB_Decr(fs);
	if ((!(({
		JB_String* _tmP403 = JB_Incr(({
			JB_String* _tmP405 = JB_Incr(JB_Str_Escape(str));
			JB_String* _tmP404 = JB_Incr(JB_Str_Unescape(_tmP405));
			JB_Decr(_tmP405);
			JB_SafeDecr(_tmP404);
			 _tmP404;
		}));
		bool _tmP402 = JB_Str_SyntaxEquals(_tmP403, str, false);
		JB_Decr(_tmP403);
		 _tmP402;
	})))) {
	...
	JB_String* natural_input = JB_Incr(JB_str_171);
	JB_String* list_input = JB_Incr(JB_str_172);
	if ((!(JB_TestCasting()))) {
		debugger;
	}
	JB_String* x = JB_Incr(JB_Str_UpperCase(JB_str_167));
	JB_Decr(x);
	LeakTester* LKT = JB_Incr(JB_Lk__New(JB_str_173));
	...
	if (JB_Str_Exists(list_input)) {
		...
		Message* list = JB_Incr(({
			Message* _tmP407 = JB_Incr(JB_Str_Parse(list_input, nil));
			Message* _tmP406 = JB_Incr(JB_Msg_ParseAST(_tmP407));
			JB_Decr(_tmP407);
			JB_SafeDecr(_tmP406);
			 _tmP406;
		}));
		...
		JB_Msg_Test(root, R, JB_str_167);
		JB_Decr(root);
		JB_Str_Print(JB_str_174);
		JB_Str_Print(R);
		JB_Decr(R);
	}
	JB_Decr(list_input);
...
Message* JB_Tk__BarThings(int Start, Syntax Syx) {
	...
	JB__Tk_StopBars = 0;
	if ((!JB_Tk__ExpectEndChar(JB_str_112, true))) {
		JB_Decr(item);
		return nil;
	}
...
Message* JB_Tk__DotSub(Syntax fn, int Start) {
	...
	if (JB_Tk__EatString(JB_str_109)) {
...
int JB_Tk__EmbeddedCode(JB_String* close, Message* dest, Syntax Syx) {
	...
	if (JB_Tk__NextStart() > result) {
		...
		JB_FS_AppendString(_fs1_fs, JB_str_140);
...
void JB_Tk__ErrorEvent2(int Start, int ExpectedBits, int RealBits) {
	...
	if (RealBits == kJB__Tk_EndOfLine) {
		...
		if (((bool)(RealBits & kJB__Tk_IllegalChar))) {
			...
			JB_String* _tmP1 = JB_Incr(({
				JB_String* _tmP395 = JB_Incr(JB_Str_Range(JB__Tk_Data, Start, N));
				JB_String* _tmP394 = JB_Incr(JB_Str_Escape(_tmP395));
				JB_Decr(_tmP395);
				JB_SafeDecr(_tmP394);
				 _tmP394;
			}));
	...
	JB_FreeIfDead(({
		JB_String* _tmP397 = JB_Incr(JB_FS_SyntaxCastString(Err));
		Message* _tmP396 = JB_Incr(JB_Tk__ErrorAdd(_tmP397, Start));
		JB_Decr(_tmP397);
		JB_SafeDecr(_tmP396);
		 _tmP396;
	}));
...
void JB_Tk__ErrorLetter(int Start) {
	JB_FreeIfDead(JB_Tk__ErrorAdd(JB_str_145, Start));
}
...
bool JB_Tk__ExpectEndChar(JB_String* s, bool Expect) {
	...
	if (Expect) {
		FastString* _fs_fs = JB_Incr(JB_FS__New());
		JB_FS_AppendString(_fs_fs, JB_str_128);
...
Message* JB_Tk__fAccess(int Start) {
	...
	JB_Tk__ParseLoop(prms, JB_str_129);
	JB_Decr(prms);
	JB_SafeDecr(result);
	return result;
}
...
Message* JB_Tk__fBackTick(int Start) {
	return JB_Tk__fStrSub(Start, JB_str_124, JB_SyxBack);
}
...
Message* JB_Tk__fBinary(int Start) {
	...
	Message* _tmP = JB_Incr(({
		JB_String* _tmP320 = JB_Incr(JB_Str_DecodeB64(S, nil));
		Message* _tmP319 = JB_Incr(JB_Tk__NewParentName(nil, JB_SyxBin, Start, _tmP320));
		JB_Decr(_tmP320);
		JB_SafeDecr(_tmP319);
		 _tmP319;
	}));
...
Message* JB_Tk__fBracket(int Start) {
	...
	if (JB_Tk__ParseLoop(Result, JB_str_132)) {
...
Message* JB_Tk__fChr(int Start) {
	...
	JB_String* name = JB_Incr(({
		JB_String* _tmP376 = JB_Incr(JB_Str_Range(JB__Tk_Data, Start + 1, After));
		JB_String* _tmP375 = JB_Incr(JB_Str_Unescape(_tmP376));
		JB_Decr(_tmP376);
		JB_SafeDecr(_tmP375);
		 _tmP375;
	}));
...
Message* JB_Tk__fComment(int Start) {
	...
	while (true) {
		i = JB_Str_Find$byteintint(d, '/', i + 1, kJB__int_max);
		if ((!JB_int_Found(i))) {
			JB_Decr(d);
			return JB_Tk__ErrorAdd(JB_str_134, Start);
...
Message* JB_Tk__fCommentLine(int Start) {
	(JB_Tk__NextStartSet(JB_Str_Find$bmintint(JB__Tk_Data, JB__Constants_CSLine, JB_Tk__NextStart(), kJB__int_max)));
	return JB__Tk_EndOfLineMarker;
}
...
Message* JB_Tk__fMsgList(int Start) {
	...
	JB_Tk__ExpectEndChar(JB_str_130, true);
	JB_SafeDecr(Result);
	return Result;
}
...
Message* JB_Tk__fShebang(int Start) {
	...
	int S2 = JB_Str_Find$bmintint(JB__Tk_Data, JB__Constants_CSLine, S, kJB__int_max);
...
Message* JB_Tk__fString(int Start) {
	...
	while (true) {
		...
		if (C0 == '{') {
			StrPos = JB_Tk__EmbeddedCode(JB_str_136, result, JB_SyxList);
		} else {
		...
		if (StrPos > After) {
			JB_Decr(result);
			return JB_Tk__ErrorAdd(JB_str_138, NewStrPos);
		}
	};
	(JB_Tk__NextStartSet(After + 1));
...
Message* JB_Tk__fStrSub(int Start, JB_String* Ender, Syntax syx) {
	...
	if ((!JB_int_Found(After))) {
		return JB_Tk__ErrorAdd(JB_str_131, JB_Tk__NextStart());
	}
...
Message* JB_Tk__fTemporalStatement(int Start) {
	...
	Message* result = nil;
	while (true) {
		Message* item = JB_Incr(JB_Tk__ProcessThing(kJB__Tk_Opp & (~kJB__Tk_OppBra), false));
		if ((!item)) {
			JB_Decr(item);
			break;
		}
		...
		if (JB_Msg_SyntaxEquals$syntaxbool(item, JB_SyxArg, false)) {
...
Message* JB_Tk__GetFuncAfter(Message* input) {
	...
	while (true) {
		...
		if ((!JB_Msg_SyntaxEquals$syntaxbool(d, JB_SyxName, false))) {
			(JB_Ring_FirstSet(d, Result));
		} else if ((!JB_Msg_SyntaxEquals$syntaxbool(Result, JB_SyxThg, false))) {
			JB_FreeIfDead(JB_Tk__ErrorAdd(JB_str_142, JB_Tk__NextStart()));
		} else {
...
void JB_Tk__Init() {
	JB_String* lines = JB_Incr(JB_str_104);
	JB_SetRef(JB__Tk_EndOfLineMarker, JB_Msg__New$syntaxstr$syntaxstr(JB_SyxYoda, lines));
	JB_Tk__Tokenz$strintparsehandler(lines, kJB__Tk_EndOfLine, JB_Tk__fEndOfLine);
	JB_Decr(lines);
	Array* numb = JB_Incr(JB_Str_ByteSplit(JB_str_105));
	JB_String* op_comp = JB_Incr(JB_str_106);
	JB_String* op_math = JB_Incr(JB_str_107);
	JB_String* PFix = JB_Incr(JB_str_108);
	TokHan* H = JB_Incr(({
		TokHan* _tmP343 = JB_Incr(({
			TokHan* _tmP345 = JB_Incr(JB_Tk__Handler(kJB__Tk_ThingWord, ((TokenHandler_fp)JB_Tk__fThingWord)));
			TokHan* _tmP344 = JB_Incr(JB_TH_Link(_tmP345, kJB__Tk_TemporalWord, ((TokenHandler_fp)JB_Tk__fTemporalStatement)));
			JB_Decr(_tmP345);
			JB_SafeDecr(_tmP344);
			 _tmP344;
		}));
		TokHan* _tmP342 = JB_Incr(JB_TH_Link(_tmP343, kJB__Tk_OppWord, ((TokenHandler_fp)JB_Tk__fOppWord)));
		JB_Decr(_tmP343);
		JB_SafeDecr(_tmP342);
		 _tmP342;
	}));
	Array* _tmP1 = JB_Incr(JB_BM_Bytes(JB__Constants_CSWordStart));
	JB_Tk__Tokens$arrayth(_tmP1, H);
	JB_Decr(_tmP1);
	JB_Decr(H);
	TokHan* _tmP2 = JB_Incr(({
		TokHan* _tmP349 = JB_Incr(({
			TokHan* _tmP351 = JB_Incr(JB_Tk__Handler(kJB__Tk_FuncAfterNoSpace, ((TokenHandler_fp)JB_Tk__fFuncCall)));
			TokHan* _tmP350 = JB_Incr(JB_TH_Link(_tmP351, kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fBracket)));
			JB_Decr(_tmP351);
			JB_SafeDecr(_tmP350);
			 _tmP350;
		}));
		TokHan* _tmP348 = JB_Incr(JB_TH_Link(_tmP349, kJB__Tk_OppBra, ((TokenHandler_fp)JB_Tk__fOppBracket)));
		JB_Decr(_tmP349);
		JB_SafeDecr(_tmP348);
		 _tmP348;
	}));
	(JB_Tk__TokenSet(JB_str_109, _tmP2));
	JB_Decr(_tmP2);
	TokHan* _tmP3 = JB_Incr(JB_Tk__Handler(kJB__Tk_colon, ((TokenHandler_fp)JB_Tk__fEndOfLine)));
	(JB_Tk__TokenSet(JB_str_110, _tmP3));
	JB_Decr(_tmP3);
	TokHan* _tmP4 = JB_Incr(({
		TokHan* _tmP355 = JB_Incr(JB_Tk__Handler(kJB__Tk_FuncAfterNoSpace, ((TokenHandler_fp)JB_Tk__fAccess)));
		TokHan* _tmP354 = JB_Incr(JB_TH_Link(_tmP355, kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fArray)));
		JB_Decr(_tmP355);
		JB_SafeDecr(_tmP354);
		 _tmP354;
	}));
	...
	JB_Decr(_tmP4);
	TokHan* _tmP5 = JB_Incr(JB_Tk__Handler(kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fMsgList)));
	(JB_Tk__TokenSet(JB_str_72, _tmP5));
	JB_Decr(_tmP5);
	TokHan* _tmP6 = JB_Incr(({
		TokHan* _tmP359 = JB_Incr(JB_Tk__Handler(kJB__Tk_FuncAfterNoSpace, ((TokenHandler_fp)JB_Tk__fDot)));
		TokHan* _tmP358 = JB_Incr(JB_TH_Link(_tmP359, kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fSDot)));
		JB_Decr(_tmP359);
		JB_SafeDecr(_tmP358);
		 _tmP358;
	}));
	(JB_Tk__TokenSet(JB_str_27, _tmP6));
	JB_Decr(_tmP6);
	TokHan* _tmP7 = JB_Incr(({
		TokHan* _tmP362 = JB_Incr(JB_Tk__Handler(kJB__Tk_FuncAfterBar, ((TokenHandler_fp)JB_Tk__fBarTypeCast)));
		TokHan* _tmP361 = JB_Incr(JB_TH_Link(_tmP362, kJB__Tk_TemporalSyx, ((TokenHandler_fp)JB_Tk__fDecl)));
		JB_Decr(_tmP362);
		JB_SafeDecr(_tmP361);
		 _tmP361;
	}));
	(JB_Tk__TokenSet(JB_str_112, _tmP7));
	JB_Decr(_tmP7);
	TokHan* _tmP8 = JB_Incr(({
		TokHan* _tmP365 = JB_Incr(JB_Tk__Handler(kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fAtName)));
		TokHan* _tmP364 = JB_Incr(JB_TH_Link(_tmP365, kJB__Tk_FuncAfterNormal, ((TokenHandler_fp)JB_Tk__fAtNamedExp)));
		JB_Decr(_tmP365);
		JB_SafeDecr(_tmP364);
		 _tmP364;
	}));
	...
	TokHan* _tmP9 = JB_Incr(({
		TokHan* _tmP368 = JB_Incr(JB_Tk__Handler(kJB__Tk_FuncAfterNormal, ((TokenHandler_fp)JB_Tk__fAdjectiveThing)));
		TokHan* _tmP367 = JB_Incr(JB_TH_Link(_tmP368, kJB__Tk_adjectiveop, ((TokenHandler_fp)JB_Tk__fAdjectiveOp)));
		JB_Decr(_tmP368);
		JB_SafeDecr(_tmP367);
		 _tmP367;
	}));
	...
	JB_Decr(_tmP9);
	JB_Tk__Tokenz$strintparsehandler(JB_str_115, kJB__Tk_ThingSyx, JB_Tk__fBeforeRelMinus);
	JB_Tk__Tokenz$strintparsehandler(PFix, kJB__Tk_ThingSyx, JB_Tk__fBeforeRel);
	JB_Tk__Tokenz$strintparsehandler(PFix, kJB__Tk_FuncAfterNormal, JB_Tk__fAfterRel);
	JB_Decr(PFix);
	JB_Tk__Tokenz$strintparsehandler(JB_str_68, kJB__Tk_ThingSyx, JB_Tk__fTemporalAsThing);
	JB_Tk__Tokenz$strintparsehandler(JB_str_116, kJB__Tk_ThingSyx, JB_Tk__fArgOpen);
	JB_Tk__Tokenz$strintparsehandler(JB_str_117, kJB__Tk_TmpOpp, JB_Tk__fTemporalRel);
	JB_Tk__Tokenz$strintparsehandler(JB_str_118, kJB__Tk_TmpOpp, JB_Tk__fTemporalRelSyx);
	JB_Tk__Tokenz$strintparsehandler(JB_str_39, kJB__Tk_ThingSyx, JB_Tk__fOpAsThing);
	JB_Tk__Tokenz$strintparsehandler(JB_str_69, kJB__Tk_ThingSyx, JB_Tk__fAnaphoricReference);
	JB_Tk__Tokenz$strintparsehandler(JB_str_71, kJB__Tk_ThingSyx, JB_Tk__fQuestion);
	JB_Tk__Tokens$arrayintparsehandler(numb, kJB__Tk_ThingSyx, JB_Tk__fNumber);
	JB_Decr(numb);
	JB_Tk__Tokenz$strintparsehandler(JB_str_67, kJB__Tk_TemporalSyx, JB_Tk__fShebang);
	JB_Tk__Tokenz$strintparsehandler(JB_str_119, kJB__Tk_EndOfLine, JB_Tk__fCommentLine);
	JB_Tk__Tokenz$strintparsehandler(JB_str_120, kJB__Tk_EndOfLine, JB_Tk__fComment);
	JB_Tk__Tokenz$strintparsehandler(op_comp, kJB__Tk_OppSyx, JB_Tk__fOppSyxNeq);
	JB_Decr(op_comp);
	JB_Tk__Tokenz$strintparsehandler(op_math, kJB__Tk_OppSyx, JB_Tk__fOppSyx);
	JB_Decr(op_math);
	JB_Tk__Tokenz$strintparsehandler(JB_str_121, kJB__Tk_OppYoda, JB_Tk__fOppYoda);
	JB_Tk__Tokenz$strintparsehandler(JB_str_122, kJB__Tk_ThingSyx, JB_Tk__fBunchOfThings);
	JB_Tk__Tokenz$strintparsehandler(JB_str_123, kJB__Tk_ThingSyx, JB_Tk__fThingSyx);
	JB_Tk__Tokenz$strintparsehandler(JB_str_51, kJB__Tk_ThingSyx, JB_Tk__fString);
	JB_Tk__Tokenz$strintparsehandler(JB_str_22, kJB__Tk_ThingSyx, JB_Tk__fChr);
	JB_Tk__Tokenz$strintparsehandler(JB_str_124, kJB__Tk_ThingSyx, JB_Tk__fBackTick);
	JB_Tk__Tokenz$strintparsehandler(JB_str_62, kJB__Tk_ThingSyx, JB_Tk__fSuperStr);
	JB_Tk__Tokenz$strintparsehandler(JB_str_125, kJB__Tk_ThingSyx, JB_Tk__fBeforeRel);
	JB_Tk__Tokenz$strintparsehandler(JB_str_95, kJB__Tk_ThingSyx, JB_Tk__fBinary);
	JB_Tk__Tokenz$strintparsehandler(JB_str_126, kJB__Tk_EndContainer, JB_Tk__fEndOfLine);
	TokHan* IllegalHandler = JB_Incr(JB_Tk__Handler(-1, ((TokenHandler_fp)JB_Tk__fError)));
	{
		int i = 0;
		while (i < 256) {
			JB_String* _tmP10 = JB_Incr(JB_Str_ChrB(i));
			(JB_Tk__TokenNilSet(_tmP10, IllegalHandler));
			JB_Decr(_tmP10);
			i++;
		};
	};
	JB_Decr(IllegalHandler);
	TokHan* _tmP11 = JB_Incr(JB_Tk__Handler(kJB__Tk_EndOfLine, ((TokenHandler_fp)JB_Tk__fIndentError)));
	(JB_Tk__TokenSet(JB_str_127, _tmP11));
	JB_Decr(_tmP11);
}
...
Message* JB_Tk__MakeRel(Message* first, int Bits) {
	...
	if (JB_Msg_SyntaxEquals$syntaxbool(opp, JB_SyxYopp, false)) {
		JB_Decr(opp);
	...
	JB_Tree_SyntaxAppend(Rel, first);
	JB_Tree_SyntaxAppend(Rel, opp);
	if (JB_Tk__EatString(JB_str_143)) {
		Rel->Func = JB_SyxERel;
	}
	bool Mode = kJB__Tk_Allow;
	if (JB_Msg_SyntaxEquals$syntaxbool(opp, JB_SyxSOpp, false)) {
		Mode = kJB__Tk_Expect;
...
int JB_Tk__MessageErrorSub(FastString* fs, int num, int ButFound) {
	if ((!JB__Tk_ErrorNames)) {
		...
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Temporal, JB_str_151));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_TemporalSyx, JB_str_152));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_TemporalWord, JB_str_153));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_EndOfLine, JB_str_148));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_colon, JB_str_154));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppSyx, JB_str_155));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppWord, JB_str_155));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Opp, JB_str_155));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppTemporal, JB_str_156));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_FuncAfterBar, JB_str_157));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Thing, JB_str_158));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_ThingWord, JB_str_158));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_ThingSyx, JB_str_159));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_IllegalChar, JB_str_160));
	}
	if (JB_FS_Length(fs)) {
		JB_FS_AppendString(fs, JB_str_161);
	} else {
		JB_FS_AppendString(fs, JB_str_77);
	}
	int BestValue = JB_Tk__FindError(num);
	if (BestValue) {
		JB_String* _tmP = JB_Incr(((JB_String*)JB_Dict_SyntaxAccess$int64(JB__Tk_ErrorNames, BestValue)));
...
Message* JB_Tk__ProcessThing(int Ops, bool Expect) {
	...
	if (((bool)NotTemp) and (!JB_Msg_SyntaxEquals$syntaxbool(result, JB_SyxArg, false))) {
...
void JB_Tk__Tokens$arrayintparsehandler(Array* arr, int bits, ParseHandler func) {
	...
	JB_Tk__Tokens$arrayth(arr, _tmP);
	JB_Decr(_tmP);
}
...
void JB_Tk__Tokenz$strintparsehandler(JB_String* s, int bits, ParseHandler func) {
	if (JB_Str_OperatorContains$str$str(s, JB_str_14)) {
		Array* _tmP = JB_Incr(JB_Str_Words(s));
		JB_Tk__Tokens$arrayintparsehandler(_tmP, bits, func);
		JB_Decr(_tmP);
	} else {
...
bool JB_Platform__linux() {
	return (({
		JB_String* _tmP225 = JB_Incr(JB_Platform());
		bool _tmP224 = JB_Str_SyntaxEquals(_tmP225, JB_str_34, false);
		JB_Decr(_tmP225);
		 _tmP224;
	}));
}
...
void JB_BinaryEscapeTest(JB_String* AllBytes) {
	...
	JB_String* a = JB_Incr(JB_str_45);
	{
		...
		while (_i1 < 10) {
			...
			JB_Decr(b);
			JB_SetRef(a, ({
				JB_String* _tmP239 = JB_Incr(JB_Str_OperatorPlus(JB_str_48, a));
				JB_String* _tmP238 = JB_Incr(JB_Str_OperatorPlus(a, _tmP239));
				JB_Decr(_tmP239);
				JB_SafeDecr(_tmP238);
				 _tmP238;
			}));
	...
	JB_Decr(a);
	JB_FS_AppendHexData$str(fs, AllBytes);
...
bool JB_CompareError(Message* expected, Message* found) {
	...
	JB_String* _tmP5 = JB_Incr(({
		Message* _tmP292 = JB_Incr(JB_Msg_PoorAnt(expected));
		JB_String* _tmP291 = JB_Incr(JB_Msg_Locate(_tmP292));
		JB_Decr(_tmP292);
		JB_SafeDecr(_tmP291);
		 _tmP291;
	}));
	...
	if (found) {
		...
		JB_String* _tmP6 = JB_Incr(({
			Message* _tmP295 = JB_Incr(JB_Msg_PoorAnt(found));
...
JB_String* JB_EntityTest() {
	...
	JB_String* result = JB_Incr(({
		JB_String* _tmP297 = JB_Incr(JB_Str_LowerCase(replicate));
		JB_String* _tmP296 = JB_Incr(JB_Str_OperatorPlus(_tmP297, JB_str_84));
		JB_Decr(_tmP297);
		JB_SafeDecr(_tmP296);
		 _tmP296;
	}));
...
int JB_Init_() {
	{
		JB_SetRef(JB_str_325, JB_Str3("Binary", 6));
		JB_SetRef(JB_str_324, JB_Str3("bin", 3));
		JB_SetRef(JB_str_323, JB_Str3("To_Do_List", 10));
		JB_SetRef(JB_str_322, JB_Str3("todo", 4));
		JB_SetRef(JB_str_321, JB_Str3("Bunch_Of_Things", 15));
		JB_SetRef(JB_str_320, JB_Str3("bnch", 4));
		JB_SetRef(JB_str_319, JB_Str3("Message_List", 12));
		JB_SetRef(JB_str_318, JB_Str3("msg", 3));
		JB_SetRef(JB_str_317, JB_Str3("Question", 8));
		JB_SetRef(JB_str_316, JB_Str3("ask", 3));
		JB_SetRef(JB_str_315, JB_Str3("yopp", 4));
		JB_SetRef(JB_str_314, JB_Str3("Yoda_Relationship", 17));
		JB_SetRef(JB_str_313, JB_Str3("yoda", 4));
		JB_SetRef(JB_str_312, JB_Str3("Typecast", 8));
		JB_SetRef(JB_str_311, JB_Str3("type", 4));
		JB_SetRef(JB_str_310, JB_Str3("Conjugate", 9));
		JB_SetRef(JB_str_309, JB_Str3("cnj", 3));
		JB_SetRef(JB_str_308, JB_Str3("Syntactic_Conjugate", 19));
		JB_SetRef(JB_str_307, JB_Str3("scnj", 4));
		JB_SetRef(JB_str_306, JB_Str3("Temporal_Reference", 18));
		JB_SetRef(JB_str_305, JB_Str3("ana", 3));
		JB_SetRef(JB_str_304, JB_Str3("Statement", 9));
		JB_SetRef(JB_str_303, JB_Str3("tmp", 3));
		JB_SetRef(JB_str_302, JB_Str3("Thing", 5));
		JB_SetRef(JB_str_301, JB_Str3("thg", 3));
		JB_SetRef(JB_str_300, JB_Str3("Syntactic_Thing", 15));
		JB_SetRef(JB_str_299, JB_Str3("sthg", 4));
		JB_SetRef(JB_str_298, JB_Str3("String", 6));
		JB_SetRef(JB_str_297, JB_Str3("str", 3));
		JB_SetRef(JB_str_296, JB_Str3("Shebang", 7));
		JB_SetRef(JB_str_295, JB_Str3("sheb", 4));
		JB_SetRef(JB_str_294, JB_Str3("Syntactic_Comment", 17));
		JB_SetRef(JB_str_293, JB_Str3("scom", 4));
		JB_SetRef(JB_str_292, JB_Str3("Self_Dot", 8));
		JB_SetRef(JB_str_291, JB_Str3("sdot", 4));
		JB_SetRef(JB_str_290, JB_Str3("Temporal_Relationship", 21));
		JB_SetRef(JB_str_289, JB_Str3("trel", 4));
		JB_SetRef(JB_str_288, JB_Str3("Relationship", 12));
		JB_SetRef(JB_str_287, JB_Str3("rel", 3));
		JB_SetRef(JB_str_286, JB_Str3("Syntactic_Operator", 18));
		JB_SetRef(JB_str_285, JB_Str3("sopp", 4));
		JB_SetRef(JB_str_284, JB_Str3("Operator", 8));
		JB_SetRef(JB_str_283, JB_Str3("opp", 3));
		JB_SetRef(JB_str_282, JB_Str3("Operator_As_Thing", 17));
		JB_SetRef(JB_str_281, JB_Str3("oat", 3));
		JB_SetRef(JB_str_280, JB_Str3("unit", 4));
		JB_SetRef(JB_str_279, JB_Str3("Number", 6));
		JB_SetRef(JB_str_278, JB_Str3("num", 3));
		JB_SetRef(JB_str_277, JB_Str3("name", 4));
		JB_SetRef(JB_str_276, JB_Str3("item", 4));
		JB_SetRef(JB_str_275, JB_Str3("Function", 8));
		JB_SetRef(JB_str_274, JB_Str3("func", 4));
		JB_SetRef(JB_str_273, JB_Str3("Equals_Relationship", 19));
		JB_SetRef(JB_str_272, JB_Str3("erel", 4));
		JB_SetRef(JB_str_271, JB_Str3("Embedded_Value", 14));
		JB_SetRef(JB_str_270, JB_Str3("emb", 3));
		JB_SetRef(JB_str_269, JB_Str3("Dot", 3));
		JB_SetRef(JB_str_268, JB_Str3("dot", 3));
		JB_SetRef(JB_str_267, JB_Str3("Declaration", 11));
		JB_SetRef(JB_str_266, JB_Str3("decl", 4));
		JB_SetRef(JB_str_265, JB_Str3("Character", 9));
		JB_SetRef(JB_str_264, JB_Str3("Before_Relationship", 19));
		JB_SetRef(JB_str_263, JB_Str3("brel", 4));
		JB_SetRef(JB_str_262, JB_Str3("Bracket", 7));
		JB_SetRef(JB_str_261, JB_Str3("bra", 3));
		JB_SetRef(JB_str_260, JB_Str3("Super_String", 12));
		JB_SetRef(JB_str_259, JB_Str3("sstr", 4));
		JB_SetRef(JB_str_258, JB_Str3("Backtick", 8));
		JB_SetRef(JB_str_257, JB_Str3("back", 4));
		JB_SetRef(JB_str_256, JB_Str3("Array", 5));
		JB_SetRef(JB_str_255, JB_Str3("arr", 3));
		JB_SetRef(JB_str_254, JB_Str3("After_Relationship", 18));
		JB_SetRef(JB_str_253, JB_Str3("arel", 4));
		JB_SetRef(JB_str_252, JB_Str3("Adjective", 9));
		JB_SetRef(JB_str_251, JB_Str3("adj", 3));
		JB_SetRef(JB_str_250, JB_Str3("Access", 6));
		JB_SetRef(JB_str_249, JB_Str3("acc", 3));
		JB_SetRef(JB_str_248, JB_Str3("Argument", 8));
		JB_SetRef(JB_str_247, JB_Str3("arg", 3));
		JB_SetRef(JB_str_246, JB_Str3("AZazÄø¬Ù", 8));
		JB_SetRef(JB_str_245, JB_Str3("\n\r,", 3));
		JB_SetRef(JB_str_244, JB_Str3(" ,:\t\n\r\\", 7));
		JB_SetRef(JB_str_243, JB_Str3("\n\r", 2));
		JB_SetRef(JB_str_242, JB_Str3("AZaz__Äø¬Ù", 10));
		JB_SetRef(JB_str_241, JB_Str3("AZaz09__Äø¬Ù", 12));
		JB_SetRef(JB_str_240, JB_Str3("\\f", 2));
		JB_SetRef(JB_str_239, JB_Str3("", 1));
		JB_SetRef(JB_str_238, JB_Str3("\\b", 2));
		JB_SetRef(JB_str_237, JB_Str3("", 1));
		JB_SetRef(JB_str_236, JB_Str3("\\&#", 3));
		...
		JB_SetRef(JB_str_230, JB_Str3("\\r", 2));
		JB_SetRef(JB_str_229, JB_Str3("\r", 1));
		JB_SetRef(JB_str_228, JB_Str3("\0", 1));
		JB_SetRef(JB_str_227, JB_Str3("[0;1;33m", 9));
		JB_SetRef(JB_str_226, JB_Str3("[0;1;32m", 9));
		JB_SetRef(JB_str_225, JB_Str3("[0;1;31m", 9));
		JB_SetRef(JB_str_224, JB_Str3("[0;1m", 6));
		JB_SetRef(JB_str_223, JB_Str3("[0m", 4));
		JB_SetRef(JB_str_222, JB_Str3("@JBSave\n", 8));
		JB_SetRef(JB_str_221, JB_Str3("unusedtype", 10));
		JB_SetRef(JB_str_220, JB_Str3("point4", 6));
		JB_SetRef(JB_str_219, JB_Str3("point3", 6));
		JB_SetRef(JB_str_218, JB_Str3("point2", 6));
		JB_SetRef(JB_str_217, JB_Str3("ipoint4", 7));
		JB_SetRef(JB_str_216, JB_Str3("ipoint3", 7));
		JB_SetRef(JB_str_215, JB_Str3("ipoint2", 7));
		JB_SetRef(JB_str_214, JB_Str3("s16", 3));
		JB_SetRef(JB_str_213, JB_Str3("u16", 3));
		JB_SetRef(JB_str_212, JB_Str3("s8", 2));
		JB_SetRef(JB_str_211, JB_Str3("f16", 3));
		JB_SetRef(JB_str_210, JB_Str3("f64", 3));
		JB_SetRef(JB_str_209, JB_Str3("u64", 3));
		JB_SetRef(JB_str_208, JB_Str3("s64", 3));
		JB_SetRef(JB_str_207, JB_Str3("u32", 3));
		JB_SetRef(JB_str_206, JB_Str3("uint", 4));
		JB_SetRef(JB_str_205, JB_Str3("int", 3));
		JB_SetRef(JB_str_204, JB_Str3("mat4", 4));
		JB_SetRef(JB_str_203, JB_Str3("mat2", 4));
		JB_SetRef(JB_str_202, JB_Str3("vec4", 4));
		JB_SetRef(JB_str_201, JB_Str3("vec3", 4));
		JB_SetRef(JB_str_200, JB_Str3("vec2", 4));
		JB_SetRef(JB_str_199, JB_Str3("double", 6));
		JB_SetRef(JB_str_198, JB_Str3("float", 5));
		JB_SetRef(JB_str_197, JB_Str3("hfloat", 6));
		JB_SetRef(JB_str_196, JB_Str3("ivec4", 5));
		JB_SetRef(JB_str_195, JB_Str3("ivec3", 5));
		JB_SetRef(JB_str_194, JB_Str3("ivec2", 5));
		JB_SetRef(JB_str_193, JB_Str3("uint64", 6));
		JB_SetRef(JB_str_192, JB_Str3("int64", 5));
		JB_SetRef(JB_str_191, JB_Str3("s16x4", 5));
		JB_SetRef(JB_str_190, JB_Str3("s16x3", 5));
		JB_SetRef(JB_str_189, JB_Str3("s16x2", 5));
		JB_SetRef(JB_str_188, JB_Str3("int16", 5));
		JB_SetRef(JB_str_187, JB_Str3("uint16", 6));
		JB_SetRef(JB_str_186, JB_Str3("sbyte4", 6));
		JB_SetRef(JB_str_185, JB_Str3("sbyte3", 6));
		JB_SetRef(JB_str_184, JB_Str3("sbyte2", 6));
		JB_SetRef(JB_str_183, JB_Str3("sbyte", 5));
		JB_SetRef(JB_str_182, JB_Str3("byte4", 5));
		JB_SetRef(JB_str_181, JB_Str3("byte3", 5));
		JB_SetRef(JB_str_180, JB_Str3("byte2", 5));
		JB_SetRef(JB_str_179, JB_Str3("char", 4));
		JB_SetRef(JB_str_178, JB_Str3("u8", 2));
		JB_SetRef(JB_str_177, JB_Str3("').", 3));
		JB_SetRef(JB_str_176, JB_Str3("' (found '", 10));
		JB_SetRef(JB_str_175, JB_Str3("Can‚Äôt parse number '", 22));
		JB_SetRef(JB_str_174, JB_Str3("Manual inspection:\n\n", 20));
		JB_SetRef(JB_str_173, JB_Str3("entities", 8));
		JB_SetRef(JB_str_172, JB_Str3("\n(tmp: \"how\"\n\t(oat: \n\t\t(opp: \"can\")\n\t\t(trel: \n\t\t\t(rel: \n\t\t\t\t(thg: \"someone\")\n\t\t\t\t(opp: \"fall\")\n\t\t\t\t(adj: \n\t\t\t\t\t(thg: \"so\")\n\t\t\t\t\t(thg: \"far\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(cnj: \"without\")\n\t\t\t(oat: \n\t\t\t\t(opp: \"realising\")\n\t\t\t\t(ana: \"it\")\n\t\t\t)\n\t\t)\n\t)\n)\n(tmp: \"story\"\n\t(thg: \"X\")\n\t(list: \n\t\t(trel: \n\t\t\t(rel: \n\t\t\t\t(thg: \"x\")\n\t\t\t\t(bra: \n\t\t\t\t\t(adj: \n\t\t\t\t\t\t(opp: \"a\")\n\t\t\t\t\t\t(opp: \"b\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t\t(thg: \"c\")\n\t\t\t)\n\t\t\t(scnj: \"-->\")\n\t\t\t(rel: \n\t\t\t\t(name: \"sonic\"\n\t\t\t\t\t(adj: \n\t\t\t\t\t\t(thg: \"the\")\n\t\t\t\t\t\t(thg: \"hedgedog\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t\t(opp: \"or\")\n\t\t\t\t(name: \"jeebox\")\n\t\t\t)\n\t\t)\n\t\t(trel: \n\t\t\t(rel: \n\t\t\t\t(thg: \"A\")\n\t\t\t\t(bra: \n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(adj: \n\t\t\t\t\t\t\t(bra: \n\t\t\t\t\t\t\t\t(adj: \n\t\t\t\t\t\t\t\t\t(opp: \"B\")\n\t\t\t\t\t\t\t\t\t(opp: \"C\")\n\t\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t(opp: \"D\")\n\t\t\t\t\t\t)\n\t\t\t\t\t\t(opp: \"E\")\n\t\t\t\t\t\t(adj: \n\t\t\t\t\t\t\t(opp: \"F\")\n\t\t\t\t\t\t\t(opp: \"G\")\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t\t(thg: \"I\")\n\t\t\t)\n\t\t\t(cnj: \"W\")\n\t\t\t(rel: \n\t\t\t\t(thg: \"A\")\n\t\t\t\t(opp: \"t\")\n\t\t\t\t(list: \n\t\t\t\t\t(item: \n\t\t\t\t\t\t(thg: \"o\")\n\t\t\t\t\t\t(thg: \"c\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t)\n)\n(tmp: \"za\"\n\t(tmp: \"xb\"\n\t\t(arg: \n\t\t\t(decl: \n\t\t\t\t(num: \"1\")\n\t\t\t\t(rel: \n\t\t\t\t\t(thg: \"x\")\n\t\t\t\t\t(opp: \"*\")\n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(thg: \"y\")\n\t\t\t\t\t\t(opp: \"*\")\n\t\t\t\t\t\t(rel: \n\t\t\t\t\t\t\t(thg: \"z\")\n\t\t\t\t\t\t\t(opp: \"=\")\n\t\t\t\t\t\t\t(brel: \"\?\"\n\t\t\t\t\t\t\t\t(arel: \"\?\"\n\t\t\t\t\t\t\t\t\t(list: )\n\t\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t\t(decl: \n\t\t\t\t(emb: )\n\t\t\t\t(rel: \n\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t(opp: \"=\")\n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(thg: \"c\")\n\t\t\t\t\t\t(opp: \"or\")\n\t\t\t\t\t\t(thg: \"d\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t\t(list: \n\t\t\t\t(thg: \"a\")\n\t\t\t)\n\t\t\t(list: \n\t\t\t\t(trel: \n\t\t\t\t\t(arr: )\n\t\t\t\t\t(scnj: \"-->\")\n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t\t(opp: \"+\")\n\t\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t\t(thg: \"b\")\n\t\t\t\t(item: \n\t\t\t\t\t(thg: \"d\")\n\t\t\t\t\t(thg: \"e\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(arr: \n\t\t\t\t(type: \n\t\t\t\t\t(thg: \"d\")\n\t\t\t\t\t(num: \"0\")\n\t\t\t\t)\n\t\t\t\t(item: \n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t\t(opp: \"+\")\n\t\t\t\t\t\t(thg: \"b\")\n\t\t\t\t\t)\n\t\t\t\t\t(thg: \"c\")\n\t\t\t\t)\n\t\t\t\t(arr: \n\t\t\t\t\t(thg: \"c\")\n\t\t\t\t\t(thg: \"d\")\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t)\n)\n(bin: \"binary encoding oof\")\n(tmp: \"function\"\n\t(thg: \"name\")\n\t(list: \n\t\t(decl: \n\t\t\t(item: \n\t\t\t\t(thg: \"x\")\n\t\t\t\t(rel: \n\t\t\t\t\t(thg: \"y\")\n\t\t\t\t\t(opp: \"+\")\n\t\t\t\t\t(thg: \"z\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(adj: \n\t\t\t\t(thg: \"a\")\n\t\t\t\t(thg: \"b\")\n\t\t\t)\n\t\t)\n\t\t(decl: \n\t\t\t(emb: )\n\t\t)\n\t)\n\t(arg: \n\t\t(item: \n\t\t\t(thg: \"label\")\n\t\t\t(func: \n\t\t\t\t(thg: \"func\")\n\t\t\t\t(list: )\n\t\t\t)\n\t\t)\n\t\t(str: \"'\\\"stri√±g test\\\"'\")\n\t\t(back: \"\\\\n\\\\(\\&#x134;\\&#xEB;\\\\C3\\\\AA\\\\C6\\\\81o\\&#9447;\\\\)\")\n\t\t(erel: \n\t\t\t(brel: \"--\"\n\t\t\t\t(arel: \"--\"\n\t\t\t\t\t(thg: \"x\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(opp: \"mod\")\n\t\t\t(char: \"ioi '\\\"\\\"'oio\")\n\t\t)\n\t\t(dot: \"y\"\n\t\t\t(dot: \"size\"\n\t\t\t\t(thg: \"y\")\n\t\t\t\t(list: \n\t\t\t\t\t(arel: \"\?\"\n\t\t\t\t\t\t(sthg: \"...\")\n\t\t\t\t\t)\n\t\t\t\t\t(sthg: \"...\")\n\t\t\t\t\t(sstr: \"s`t`r\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(list: \n\t\t\t\t(brel: \"-\"\n\t\t\t\t\t(thg: \"z\")\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t\t(rel: \n\t\t\t(thg: \"x\")\n\t\t\t(opp: \"=\")\n\t\t\t(rel: \n\t\t\t\t(str: \n\t\t\t\t\t(emb: \"{ab\")\n\t\t\t\t\t(thg: \"x\")\n\t\t\t\t\t(emb: \" \")\n\t\t\t\t\t(thg: \"y\")\n\t\t\t\t\t(thg: \"z\")\n\t\t\t\t\t(emb: \"ab}\")\n\t\t\t\t)\n\t\t\t\t(opp: \"+\")\n\t\t\t\t(back: \"abc\")\n\t\t\t)\n\t\t)\n\t\t(erel: \n\t\t\t(type: \n\t\t\t\t(thg: \"z\")\n\t\t\t\t(list: \n\t\t\t\t\t(thg: \"a\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(opp: \"+\")\n\t\t\t(sdot: \"exit\"\n\t\t\t\t(list: \n\t\t\t\t\t(item: \n\t\t\t\t\t\t(thg: \"code\")\n\t\t\t\t\t\t(num: \"-3\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t\t(tmp: \"for\"\n\t\t\t(list: \n\t\t\t\t(item: \n\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t(thg: \"b\")\n\t\t\t\t)\n\t\t\t)\n\t\t\t(arg: \n\t\t\t\t(rel: \n\t\t\t\t\t(brel: \"$\"\n\t\t\t\t\t\t(arel: \"$\"\n\t\t\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t\t(opp: \"=\")\n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(brel: \"&\"\n\t\t\t\t\t\t\t(arel: \"&\"\n\t\t\t\t\t\t\t\t(thg: \"b\")\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t)\n\t\t\t\t\t\t(opp: \"+\")\n\t\t\t\t\t\t(brel: \"!\"\n\t\t\t\t\t\t\t(arel: \"!\"\n\t\t\t\t\t\t\t\t(thg: \"c\")\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t\t(tmp: \"if\"\n\t\t\t(rel: \n\t\t\t\t(bra: \n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(char: \"a\")\n\t\t\t\t\t\t(opp: \"&&&\")\n\t\t\t\t\t\t(rel: \n\t\t\t\t\t\t\t(thg: \"b\")\n\t\t\t\t\t\t\t(opp: \"===\")\n\t\t\t\t\t\t\t(rel: \n\t\t\t\t\t\t\t\t(brel: \"!\"\n\t\t\t\t\t\t\t\t\t(brel: \"!\"\n\t\t\t\t\t\t\t\t\t\t(brel: \"!\"\n\t\t\t\t\t\t\t\t\t\t\t(thg: \"b\")\n\t\t\t\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t\t(opp: \"|||\")\n\t\t\t\t\t\t\t\t(thg: \"c\")\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t\t(opp: \"^^^\")\n\t\t\t\t(bra: \n\t\t\t\t\t(rel: \n\t\t\t\t\t\t(thg: \"a\")\n\t\t\t\t\t\t(opp: \"~>=\")\n\t\t\t\t\t\t(thg: \"b\")\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t\t(arg: \n\t\t\t\t(tmp: \"return\"\n\t\t\t\t\t(tmp: \"return\"\n\t\t\t\t\t\t(rel: \n\t\t\t\t\t\t\t(unit: \"cm\"\n\t\t\t\t\t\t\t\t(num: \"0xff_\")\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t\t(opp: \"+\")\n\t\t\t\t\t\t\t(unit: \"miles\"\n\t\t\t\t\t\t\t\t(num: \"10_\")\n\t\t\t\t\t\t\t)\n\t\t\t\t\t\t)\n\t\t\t\t\t)\n\t\t\t\t)\n\t\t\t)\n\t\t)\n\t)\n)\n", 4078));
		JB_SetRef(JB_str_171, JB_Str3("// Jeebox self-test file.\n// Tests all syntax in small amount of code.\n// Does not make sense in any other way.\n\nhow %can someone fall so~far #without %realising ‚Äìit\n\n\nstory X (\n\tx (a ~ b) c\n\t\t\t-->\n\tsonic@the~hedgedog or @jeebox\n\n\tA ((B~C)~D E F~G) I #W A t (o:c)\n)\n\n\n\n\nza::xb {\n\t|1| x * y * z = \?()\?\n\t|| a = c or d\n\t(a,)\n\t([] --> a + a,b,d:e)\n\t[d|0|, a+b:c, [c, d]] /* /*\n\t\tnested comments!\n\t\t*/ I found it useful.*/\n\n\t// }\n}\n\n#(OcbkON9v86LkOszaQMvd86zlPW)#\n\nfunction name (|x:y+z| a ~ b, ||) {\n\tlabel: func()\n\t\"'\\\"stri\\c3\\b1g test\\\"'\"\n\t`\\n\\(&#x134;&#xEB;\\C3\\AA\\C6\\81o&#9447;\\)`\n\t--x-- mod= 'ioi \\'\"\"\\'oio'\n\t\n\ty.size(...\?, ..., <(`s`t`r`)>).y(-z)\n\n\tx = \"{ab$\{x} $\{y}$\{z}ab}\" + `abc`\n\tz|(a,)| += .exit(code: -3)\n\n\tfor (a : b) {\n\t\t\$a$ = &b& + !c!\n\t}\n\n\tif ('a' &&& b === !!!b ||| c) ^^^ (a ~>= b) {\n\t\treturn::return 0xff_cm + 10_miles\n\t}\n}\n\n\n", 842));
		JB_SetRef(JB_str_170, JB_Str3(": Failed\n\n", 10));
		JB_SetRef(JB_str_169, JB_Str3("Test ", 5));
		JB_SetRef(JB_str_168, JB_Str3("list", 4));
		JB_SetRef(JB_str_167, JB_Str3("normal", 6));
		JB_SetRef(JB_str_166, JB_Str3(" in ", 4));
		JB_SetRef(JB_str_165, JB_Str3("' at byte: ", 11));
		JB_SetRef(JB_str_164, JB_Str3(": '", 3));
		JB_SetRef(JB_str_163, JB_Str3("(nil)", 5));
		JB_SetRef(JB_str_162, JB_Str3("something else", 14));
		JB_SetRef(JB_str_161, JB_Str3(" / ", 3));
		JB_SetRef(JB_str_160, JB_Str3("illegal-character", 17));
		JB_SetRef(JB_str_159, JB_Str3("something", 9));
		JB_SetRef(JB_str_158, JB_Str3("a name", 6));
		JB_SetRef(JB_str_157, JB_Str3("'|'", 3));
		JB_SetRef(JB_str_156, JB_Str3("temporal-operator", 17));
		JB_SetRef(JB_str_155, JB_Str3("operator", 8));
		JB_SetRef(JB_str_154, JB_Str3("':'", 3));
		JB_SetRef(JB_str_153, JB_Str3("statement", 9));
		JB_SetRef(JB_str_152, JB_Str3("declaration", 11));
		JB_SetRef(JB_str_151, JB_Str3("temporal", 8));
		JB_SetRef(JB_str_150, JB_Str3("Found illegal-character ", 24));
		JB_SetRef(JB_str_149, JB_Str3("end-of-data", 11));
		JB_SetRef(JB_str_148, JB_Str3("end-of-line", 11));
		JB_SetRef(JB_str_147, JB_Str3("Unexpected ", 11));
		JB_SetRef(JB_str_146, JB_Str3("Indent with tabs only. (A later jeebox will allow spaces or tabs, but only one type per-file.)", 94));
		JB_SetRef(JB_str_145, JB_Str3("Expected a letter (aA-zZ, '_', etc).", 36));
		JB_SetRef(JB_str_144, JB_Str3("Can‚Äôt use temporals like this.", 32));
		JB_SetRef(JB_str_143, JB_Str3("=", 1));
		JB_SetRef(JB_str_142, JB_Str3("Must be a proper name (a-z or letters)", 38));
		JB_SetRef(JB_str_141, JB_Str3("' not allowed.", 14));
		JB_SetRef(JB_str_140, JB_Str3("Nested '", 8));
		JB_SetRef(JB_str_139, JB_Str3("#/", 2));
		JB_SetRef(JB_str_138, JB_Str3("Embedded expression didn't close.", 33));
		JB_SetRef(JB_str_137, JB_Str3("String didn't close!", 20));
		JB_SetRef(JB_str_136, JB_Str3("}", 1));
		JB_SetRef(JB_str_135, JB_Str3("Can‚Äôt have '=' after this operator.", 37));
		JB_SetRef(JB_str_134, JB_Str3("Comment did not close.", 22));
		JB_SetRef(JB_str_133, JB_Str3("Char didn't close!", 18));
		JB_SetRef(JB_str_132, JB_Str3(")", 1));
		JB_SetRef(JB_str_131, JB_Str3("String did not close.", 21));
		JB_SetRef(JB_str_130, JB_Str3("¬ª", 2));
		JB_SetRef(JB_str_129, JB_Str3("]", 1));
		JB_SetRef(JB_str_128, JB_Str3("Missing '", 9));
		JB_SetRef(JB_str_127, JB_Str3("\n ", 2));
		JB_SetRef(JB_str_126, JB_Str3("] ) } #/ ¬ª", 11));
		JB_SetRef(JB_str_125, JB_Str3("~~~", 3));
		JB_SetRef(JB_str_124, JB_Str3("`", 1));
		JB_SetRef(JB_str_123, JB_Str3("...", 3));
		JB_SetRef(JB_str_122, JB_Str3("‚Ä°", 3));
		JB_SetRef(JB_str_121, JB_Str3("¬¨", 2));
		JB_SetRef(JB_str_120, JB_Str3("/*", 2));
		JB_SetRef(JB_str_119, JB_Str3("//", 2));
		JB_SetRef(JB_str_118, JB_Str3("--> <-- <-->", 12));
		JB_SetRef(JB_str_117, JB_Str3("#", 1));
		JB_SetRef(JB_str_116, JB_Str3("{", 1));
		JB_SetRef(JB_str_115, JB_Str3("- *", 3));
		...
		JB_SetRef(JB_str_108, JB_Str3("++ -- $ & ! \?", 13));
		JB_SetRef(JB_str_107, JB_Str3("* / + - ^ >> << >>> <<< ||| &&& ^^^", 35));
		JB_SetRef(JB_str_106, JB_Str3("= == === < > <= <== >= >== != !== <=> ~>= ~<= ~> ~< ~= ~!= ~<=> :=", 66));
		JB_SetRef(JB_str_105, JB_Str3("0123456789", 10));
		JB_SetRef(JB_str_104, JB_Str3("\r \n ,", 5));
		JB_SetRef(JB_str_103, JB_Str3("‚Äù", 3));
		JB_SetRef(JB_str_102, JB_Str3(" ‚Äú", 4));
		...
		JB_SetRef(JB_str_73, JB_Str3(" ¬ª", 3));
		...
		JB_SetRef(JB_str_59, JB_Str3("Can‚Äôt find syntax: ", 21));
		...
		JB_SetRef(JB_str_21, JB_Str3("Can‚Äôt find '", 14));
		...
		JB_SyxArg = JB_Syntax__StdNew(JB_Msg_Arg__, JB_str_247, JB_str_248);
		JB_SyxAcc = JB_Syntax__StdNew(JB_Msg_Acc__, JB_str_249, JB_str_250);
		JB_SyxAdj = JB_Syntax__StdNew(JB_Msg_Adj__, JB_str_251, JB_str_252);
		JB_SyxARel = JB_Syntax__StdNew(JB_Msg_ARel__, JB_str_253, JB_str_254);
		JB_SyxArr = JB_Syntax__StdNew(JB_Msg_Arr__, JB_str_255, JB_str_256);
		JB_SyxBack = JB_Syntax__StdNew(JB_Msg_Back__, JB_str_257, JB_str_258);
		JB_SyxSStr = JB_Syntax__StdNew(JB_Msg_SStr__, JB_str_259, JB_str_260);
		JB_SyxBra = JB_Syntax__StdNew(JB_Msg_Bra__, JB_str_261, JB_str_262);
		JB_SyxBRel = JB_Syntax__StdNew(JB_Msg_BRel__, JB_str_263, JB_str_264);
		JB_SyxChar = JB_Syntax__StdNew(JB_Msg_Char__, JB_str_179, JB_str_265);
		JB_SyxDecl = JB_Syntax__StdNew(JB_Msg_Decl__, JB_str_266, JB_str_267);
		JB_SyxDot = JB_Syntax__StdNew(JB_Msg_Dot__, JB_str_268, JB_str_269);
		JB_SyxEmb = JB_Syntax__StdNew(JB_Msg_Emb__, JB_str_270, JB_str_271);
		JB_SyxERel = JB_Syntax__StdNew(JB_Msg_ERel__, JB_str_272, JB_str_273);
		JB_SyxFunc = JB_Syntax__StdNew(JB_Msg_Func__, JB_str_274, JB_str_275);
		JB_SyxItem = JB_Syntax__StdNew(JB_Msg_Item__, JB_str_276, JB_str_0);
		JB_SyxList = JB_Syntax__StdNew(JB_Msg_List__, JB_str_168, JB_str_0);
		JB_SyxName = JB_Syntax__StdNew(JB_Msg_Name__, JB_str_277, JB_str_0);
		JB_SyxNum = JB_Syntax__StdNew(JB_Msg_Num__, JB_str_278, JB_str_279);
		JB_SyxUnit = JB_Syntax__StdNew(JB_Msg_Unit__, JB_str_280, JB_str_0);
		JB_SyxOat = JB_Syntax__StdNew(JB_Msg_Oat__, JB_str_281, JB_str_282);
		JB_SyxOpp = JB_Syntax__StdNew(JB_Msg_Opp__, JB_str_283, JB_str_284);
		JB_SyxSOpp = JB_Syntax__StdNew(JB_Msg_SOpp__, JB_str_285, JB_str_286);
		JB_SyxRel = JB_Syntax__StdNew(JB_Msg_Rel__, JB_str_287, JB_str_288);
		JB_SyxTRel = JB_Syntax__StdNew(JB_Msg_TRel__, JB_str_289, JB_str_290);
		JB_SyxSDot = JB_Syntax__StdNew(JB_Msg_SDot__, JB_str_291, JB_str_292);
		JB_SyxSCom = JB_Syntax__StdNew(JB_Msg_SCom__, JB_str_293, JB_str_294);
		JB_SyxSheb = JB_Syntax__StdNew(JB_Msg_Sheb__, JB_str_295, JB_str_296);
		JB_SyxStr = JB_Syntax__StdNew(JB_Msg_Str__, JB_str_297, JB_str_298);
		JB_SyxSThg = JB_Syntax__StdNew(JB_Msg_SThg__, JB_str_299, JB_str_300);
		JB_SyxThg = JB_Syntax__StdNew(JB_Msg_Thg__, JB_str_301, JB_str_302);
		JB_SyxTmp = JB_Syntax__StdNew(JB_Msg_Tmp__, JB_str_303, JB_str_304);
		JB_SyxAna = JB_Syntax__StdNew(JB_Msg_Ana__, JB_str_305, JB_str_306);
		JB_SyxSCnj = JB_Syntax__StdNew(JB_Msg_SCnj__, JB_str_307, JB_str_308);
		JB_SyxCnj = JB_Syntax__StdNew(JB_Msg_Cnj__, JB_str_309, JB_str_310);
		JB_SyxType = JB_Syntax__StdNew(JB_Msg_Type__, JB_str_311, JB_str_312);
		JB_SyxYoda = JB_Syntax__StdNew(JB_Msg_Yoda__, JB_str_313, JB_str_314);
		JB_SyxYopp = JB_Syntax__StdNew(JB_Msg_Yopp__, JB_str_315, JB_str_0);
		JB_SyxAsk = JB_Syntax__StdNew(JB_Msg_Ask__, JB_str_316, JB_str_317);
		JB_SyxMsg = JB_Syntax__StdNew(JB_Msg_Msg__, JB_str_318, JB_str_319);
		JB_SyxBnch = JB_Syntax__StdNew(JB_Msg_Bnch__, JB_str_320, JB_str_321);
		JB_SyxTodo = JB_Syntax__StdNew(JB_Msg_Todo__, JB_str_322, JB_str_323);
		JB_SyxBin = JB_Syntax__StdNew(JB_Msg_Bin__, JB_str_324, JB_str_325);
	};
	//// ErrorColors;
...
bool JB_TestCasting() {
	...
	DTWrap* _tmP = JB_Incr(JB_Wrap__New$int64(((int)({
		DTWrap* _tmP300 = JB_Incr(((DTWrap*)JB_Dict_SyntaxAccess$str(d2, JB_str_85)));
		int64 _tmP299 = JB_Wrap_Value(_tmP300);
		JB_Decr(_tmP300);
		 _tmP299;
	})) + 1));
...
void JB_AstUtil_UseLayer(AstUtil* self, Message* src) {
	...
	JB_MemoryLayer* L = JB_Incr(({
		JB_String* _tmP280 = JB_Incr(JB_Str_LowerCase(src->Name));
		JB_MemoryLayer* _tmP279 = JB_Incr(((JB_MemoryLayer*)JB_Dict_SyntaxAccess$str(self->Mem, _tmP280)));
		JB_Decr(_tmP280);
		JB_SafeDecr(_tmP279);
		 _tmP279;
	}));
...
JB_String* JB_int_Render$fs(int self, FastString* fs_in) {
	FastString* fs = JB_Incr(JB_FS__FastNew(fs_in));
	JB_FS_SyntaxAppend$int(fs, self);
	JB_String* _tmP = JB_Incr(JB_FS_SmartResult(fs, fs_in));
	JB_Decr(fs);
	JB_SafeDecr(_tmP);
	return _tmP;
...
JB_String* JB_int64_Render(int64 self, FastString* fs_in) {
	FastString* fs = JB_Incr(JB_FS__FastNew(fs_in));
	JB_FS_SyntaxAppend$int64(fs, self);
	JB_String* _tmP = JB_Incr(JB_FS_SmartResult(fs, fs_in));
	JB_Decr(fs);
	JB_SafeDecr(_tmP);
	return _tmP;
...
JB_String* JB_iPoint2_Render(iPoint2* self, FastString* fs_in) {
	FastString* fs = JB_Incr(JB_FS__FastNew(fs_in));
	JB_FS_AppendByte(fs, '(');
	JB_FS_SyntaxAppend$int(fs, self->X);
	JB_FS_AppendByte(fs, ',');
	JB_FS_AppendByte(fs, ' ');
	JB_FS_SyntaxAppend$int(fs, self->Y);
	JB_FS_AppendByte(fs, ')');
	JB_String* _tmP = JB_Incr(JB_FS_SmartResult(fs, fs_in));
	JB_Decr(fs);
	JB_SafeDecr(_tmP);
...
bool JB_Loader_ItemIsInt(ObjectLoader* self) {
	Message* C = self->CurrItem;
	return ((JB_Msg_SyntaxEquals$syntaxbool(((Message*)JB_Ring_Last(C)), JB_SyxNum, false)));
}
...
JB_Object* JB_Loader_ObjectSub(ObjectLoader* self, Message* c) {
	if (JB_Msg_SyntaxEquals$syntaxbool(c, JB_SyxThg, false)) {
		return JB_Dict_SyntaxAccess$str(self->Dict, c->Name);
	} else if (JB_Msg_SyntaxEquals$syntaxbool(c, JB_SyxStr, false)) {
...
void JB_Saver_AppendInt(ObjectSaver* self, int64 i) {
	JB_FS_SyntaxAppend$int64(self->Dest, i);
...
void JB_Saver_AppendString(ObjectSaver* self, JB_String* s) {
	if (JB_Str_Exists(s) and JB_int_Found(JB_Str_Find$byteintint(s, '`', 0, kJB__int_max))) {
...
JB_String* JB_Syntax_Render(Syntax self, FastString* fs_in) {
	...
	if (self) {
		JB_FreeIfDead(({
			SyntaxObj* _tmP283 = JB_Incr(JB_Syntax_Obj(self));
			JB_String* _tmP282 = JB_Incr(JB_Fn_Render(_tmP283, fs));
			JB_Decr(_tmP283);
			JB_SafeDecr(_tmP282);
			 _tmP282;
		}));
...
JB_String* JB_Wrap_Render(DTWrap* self, FastString* fs_in) {
	...
	if (JB_DataTypeCode_IsFloat(self->DataType)) {
		...
		JB_FS_SyntaxAppend$int64(fs, self->_PrivValue);
	}
	JB_String* _tmP = JB_Incr(JB_FS_SmartResult(fs, fs_in));
	JB_Decr(fs);
	JB_SafeDecr(_tmP);
...
DTWrap* JB_Wrap__New$int64(int64 v) {
	...
	if (Result) {
		JB_Wrap_Constructor$int64(Result, v);
	}
	return Result;
}
...
void JB_FS_SyntaxAppend$int(FastString* self, int data) {
	JB_FS_SyntaxAppend$int64(self, ((int64)data));
}
...
Array* JB_Str_ByteSplit(JB_String* self) {
	Array* result = JB_Incr(JB_Array__New0());
	{
		...
		int n = 0;
		while (n < _LoopSrc1) {
			JB_String* _tmP2 = JB_Incr(JB_Str_ChrB(JB_Str_SyntaxAccess(self, n)));
			JB_Array_Append(result, _tmP2);
			JB_Decr(_tmP2);
			n++;
		};
...
Message* JB_Str_Parse(JB_String* self, Message* into) {
	...
	JB__Tk_ErrNode = ({
		Message* __imP = JB_Incr((JB_Syntax_Msg$str$str(JB_SyxArg, JB_str_0)));
		JB_SafeDecr(__imP);
		 __imP;
	});
	if ((!(into))) {
...
Message* JB_Str_ParseAs(JB_String* self, JB_String* name) {
	if ((!JB_Str_Exists(name)) or (JB_Str_SyntaxEquals(name, JB_str_167, true))) {
		return JB_Str_Parse(self, nil);
	} else if (JB_Str_SyntaxEquals(name, JB_str_168, true)) {
		return ({
			Message* _tmP401 = JB_Incr(JB_Str_Parse(self, nil));
			Message* _tmP400 = JB_Incr(JB_Msg_ParseAST(_tmP401));
			JB_Decr(_tmP401);
			JB_SafeDecr(_tmP400);
			 _tmP400;
		});
...
int JB_FI_NextSep(StringFields* self, int Prev) {
	return JB_Str_Find$byteintint(self->Data, self->Sep, Prev, kJB__int_max);
}
...
void JB_Dict_LoadProperties(Dictionary* self, ObjectLoader* Loader) {
	while (JB_Loader_HasItem(Loader)) {
		...
		if (JB_Loader_ItemIsInt(Loader)) {
			(JB_Dict_ValueSet(self, name, JB_Wrap__New$int64(JB_Loader_ItemInt(Loader))));
		} else {
...
JB_Object* JB_Dict_Syntax(Dictionary* self, JB_String* Key) {
	return ({
		JB_String* _tmP325 = JB_Incr(JB_Str_FastLower(Key));
		JB_Object* _tmP324 = JB_Incr(JB_Dict_SyntaxAccess$str(self, _tmP325));
		JB_Decr(_tmP325);
		JB_SafeDecr(_tmP324);
		 _tmP324;
	});
...
JB_Object* JB_Dict_SyntaxAccess$int64(Dictionary* self, int64 Key) {
	return JB_Dict_SyntaxAccess$str(self, JB_int64_DictName(Key));
}
...
void JB_LinkedList_LoadProperties(JB_LinkedList* self, ObjectLoader* Loader) {
	JB_Loader_LinkedList$linkedlist(Loader, (&self->_Next));
...
void JB_Err_GrabLine(JB_Error* self, FastString* fs, bool Usecolor) {
	...
	int Start = JB_Str_Find$bmintint(d, JB__Constants_CSLine, self->Position, 0) + 1;
	int Finish = JB_Str_Find$bmintint(d, JB__Constants_CSLine, self->Position, kJB__int_max);
	if (Finish == -1) {
		Finish = (JB_Str_Length(d) + 1);
	}
	JB_String* _tmP = JB_Incr(({
		JB_String* _tmP195 = JB_Incr(JB_Str_Range(d, Start, Finish));
		JB_String* _tmP194 = JB_Incr(JB_Str_ReplaceAll(_tmP195, JB_str_16, JB_str_14, false, nil));
		JB_Decr(_tmP195);
		JB_SafeDecr(_tmP194);
		 _tmP194;
	}));
...
bool JB_Err_LineIdentifiers(JB_Error* self, FastString* fs, JB_String* path) {
	...
	JB_FS_SyntaxAppend$int(fs, JB_Err_LinePos(self, d));
	JB_FS_AppendByte(fs, ':');
	int Start = JB_Str_Find$bmintint(d, JB__Constants_CSLine, self->Position, 0);
	JB_Decr(d);
	JB_FS_SyntaxAppend$int(fs, (self->Position - Start));
	JB_FS_AppendByte(fs, ':');
	return true;
}
...
int JB_Err_LinePos(JB_Error* self, JB_String* data) {
	if (JB_Err_HasPosition(self)) {
		return ({
			JB_String* _tmP192 = JB_Incr(JB_Str_Range(data, 0, self->Position));
			int _tmP191 = JB_Str_LineCount(_tmP192);
			JB_Decr(_tmP192);
			 _tmP191;
		});
	}
...
void JB_Rec_PrintErrors(JB_ErrorReceiver* self, int Flags) {
	...
	FastString* _fs_fs = JB_Incr(JB_FS__New());
	JB_FS_SyntaxAppend$int(_fs_fs, Count);
...
void JB_Msg_Acc__(Message* self, FastString* fs) {
	...
	JB_FS_SyntaxAppend$msg(fs, _tmP);
	JB_Decr(_tmP);
...
void JB_Msg_Adj__(Message* self, FastString* fs) {
	...
	JB_FS_AppendByte(fs, '~');
	Message* _tmP1 = JB_Incr(((Message*)JB_Ring_Last(self)));
	JB_FS_SyntaxAppend$msg(fs, _tmP1);
	JB_Decr(_tmP1);
}
...
void JB_Msg_Ask__(Message* self, FastString* fs) {
	...
	JB_FS_SyntaxAppend$msg(fs, op);
	JB_FS_AppendByte(fs, ' ');
	JB_FS_SyntaxAppend$msg(fs, first);
	JB_Decr(first);
	Message* last = JB_Incr(((Message*)JB_Ring_NextSib(op)));
	JB_Decr(op);
	if (last) {
		JB_FS_AppendByte(fs, ' ');
		JB_FS_SyntaxAppend$msg(fs, last);
	}
	JB_Decr(last);
}
...
JB_String* JB_Msg_AST(Message* self, FastString* fs_in) {
	FastString* fs = JB_Incr(JB_FS__FastNew(fs_in));
	bool PrintSelf = (((bool)JB_FreeIfDead(((Message*)JB_Ring_Parent(self)))) or (!JB_Msg_SyntaxEquals$syntaxbool(self, JB_SyxArg, false)));
	if (PrintSelf) {
		JB_FS_AppendByte(fs, '(');
...
void JB_Msg_Back__(Message* self, FastString* fs) {
	if (JB_int_Found(JB_Str_Find$byteintint(self->Name, '`', 0, kJB__int_max))) {
...
Message* JB_Msg_Copy(Message* self, bool DefaultLayer) {
	if (DefaultLayer) {
		...
		return ({
			JB_MemoryLayer* _tmP246 = JB_Incr(JB_ObjLayer(self));
			Message* _tmP245 = JB_Incr(JB_Msg_LayerCopy(self, _tmP246));
			JB_Decr(_tmP246);
			JB_SafeDecr(_tmP245);
			 _tmP245;
		});
...
Message* JB_Msg_FindFlat(Message* self, Syntax s, JB_String* name, bool Err) {
	{
		...
		while (ch) {
			Message* _N1 = JB_Incr(((Message*)JB_Ring_NextSib(ch)));
			if (((JB_Msg_SyntaxEquals$syntaxbool(ch, s, false))) and ((JB_Msg_SyntaxEquals$strbool(ch, name, false)))) {
				JB_Decr(_N1);
				JB_SafeDecr(ch);
				return ch;
			}
	...
	if (Err) {
		JB_Msg_AccessErr$syntaxstr(self, s, name);
	}
	return nil;
}
...
Message* JB_Msg_FixTRels(Message* self, Message* Last) {
	if ((!Last)) {
	} else if ((!JB_Msg_SyntaxEquals$syntaxbool(Last, JB_SyxTRel, false))) {
...
JB_String* JB_Msg_FuncName(Message* self) {
	return ({
		JB_String* _tmP272 = JB_Incr(JB_Syntax_LongName(self->Func));
		JB_String* _tmP271 = JB_Incr(JB_Str_LowerCase(_tmP272));
		JB_Decr(_tmP272);
		JB_SafeDecr(_tmP271);
		 _tmP271;
	});
...
void JB_Msg_List__(Message* self, FastString* fs) {
	...
	int count = 0;
	{
		...
		while (ch) {
			...
			} else if ((JB_Msg_SyntaxEquals$syntaxbool(ch, JB_SyxItem, false)) or (JB_Msg_SyntaxEquals$syntaxbool(ch, JB_SyxDecl, false))) {
				count = 2;
			}
			JB_FS_SyntaxAppend$msg(fs, ch);
			ch = _N1;
	...
	if (count == 1) {
		if ((!((Message*)JB_Ring_NextSib(self))) and (((JB_Msg_SyntaxEquals$syntaxbool(((Message*)JB_Ring_Parent(self)), JB_SyxDot, false))) or ((JB_Msg_SyntaxEquals$syntaxbool(((Message*)JB_Ring_Parent(self)), JB_SyxFunc, false))))) {
		} else {
			JB_FS_AppendByte(fs, ',');
		}
	}
...
JB_String* JB_Msg_Locate(Message* self) {
	JB_String* fname = JB_Incr(JB_str_163);
	if ((!self)) {
		JB_SafeDecr(fname);
		return fname;
	}
	...
	JB_String* rs = JB_Incr(JB_str_163);
	...
	JB_FS_SyntaxAppend$int(_fs_fs, self->Position);
...
Message* JB_Msg_ParseAST(Message* self) {
	...
	Message* Result = JB_Incr((JB_Syntax_Msg$str$str(JB_SyxArg, JB_str_0)));
	Message* _tmP = JB_Incr(((Message*)JB_Ring_First(self)));
...
void JB_Msg_ParseAST_(Message* self, Message* Src, AstUtil* Util) {
	if (JB_Msg_SyntaxEquals$syntaxbool(Src, JB_SyxNum, false)) {
		self->Position = JB_Msg_Int(Src);
	} else if (JB_Msg_SyntaxEquals$syntaxbool(Src, JB_SyxStr, false)) {
	...
	} else {
		{
			...
			while (line) {
				...
				Message* children = JB_Incr(({
					Message* _tmP336 = JB_Incr(((Message*)JB_Ring_First(MsgDesc)));
					Message* _tmP335 = JB_Incr(((Message*)JB_Ring_NextSib(_tmP336)));
					JB_Decr(_tmP336);
					JB_SafeDecr(_tmP335);
					 _tmP335;
				}));
				...
				Message* itm = JB_Incr(JB_Msg_SyntaxAccess$syntax$syntax$syntax(MsgDesc, JB_SyxItem));
				JB_Decr(MsgDesc);
				if ((!itm)) {
					JB_Decr(line);
					JB_Decr(children);
				...
				Message* Result = JB_Incr(({
					JB_String* _tmP338 = JB_Incr(({
						Message* _tmP340 = JB_Incr(((Message*)JB_Ring_NextSib(func)));
						JB_String* _tmP339 = JB_Incr(JB_Msg_Nyme(_tmP340));
						JB_Decr(_tmP340);
						JB_SafeDecr(_tmP339);
						 _tmP339;
					}));
					Message* _tmP337 = JB_Incr(JB_Msg__NewParser(self, fn->ID, self->Position, _tmP338));
					JB_Decr(_tmP338);
					JB_SafeDecr(_tmP337);
					 _tmP337;
				}));
...
JB_String* JB_Msg_Render(Message* self, FastString* fs_in) {
	FastString* fs = JB_Incr(JB_FS__FastNew(fs_in));
	JB_FS_SyntaxAppend$msg(fs, self);
	JB_String* _tmP = JB_Incr(JB_FS_SmartResult(fs, fs_in));
	JB_Decr(fs);
	JB_SafeDecr(_tmP);
	return _tmP;
...
void JB_Msg_Str__(Message* self, FastString* fs) {
	...
	JB_FS_AppendEscape(fs, self->Name);
	{
		...
		while (ch) {
			Message* _N1 = JB_Incr(((Message*)JB_Ring_NextSib(ch)));
			if (JB_Msg_SyntaxEquals$syntaxbool(ch, JB_SyxEmb, false)) {
...
Message* JB_Msg_SyntaxAccess$syntax$syntax$syntax(Message* self, Syntax s) {
	...
	JB_Msg_AccessErr$syntaxstr(ch, s, nil);
	return nil;
}
...
void JB_Msg_Tmp__(Message* self, FastString* fs) {
	if (((!JB_Msg_SyntaxEquals$syntaxbool(((Message*)JB_Ring_Parent(self)), JB_SyxArg, false))) and ((!JB_Msg_SyntaxEquals$syntaxbool(((Message*)JB_Ring_PrevSib(self)), JB_SyxArg, false)))) {
...
void JB_Msg_UnEmbed(Message* self) {
	...
	if ((!(((JB_Msg_SyntaxEquals$syntaxbool(s1, JB_SyxEmb, false))) and (!JB_FreeIfDead(((Message*)JB_Ring_NextSib(s1))))))) {
		JB_Decr(s1);
		return;
	}
	JB_Tree_Remove(s1);
...
void JB_Msg_Yoda__(Message* self, FastString* fs) {
	...
	Message* _tmP2 = JB_Incr(({
		Message* _tmP266 = JB_Incr(((Message*)JB_Ring_First(self)));
		Message* _tmP265 = JB_Incr(((Message*)JB_Ring_NextSib(_tmP266)));
		JB_Decr(_tmP266);
		JB_SafeDecr(_tmP265);
		 _tmP265;
	}));
	JB_FS_SyntaxAppend$msg(fs, _tmP2);
	JB_Decr(_tmP2);
}
...
Message* JB_Msg__New$syntaxstr$syntaxstr(Syntax Func, JB_String* Name) {
	Message* Result = JB_Msg__Alloc();
	if (Result) {
		JB_Msg_Constructor$syntaxstr$syntaxstr(Result, Func, Name);
	}
	return Result;
}
...
__lib__ Message* jb_msg_create(Message* self, Syntax Type, JB_String* Name) {
	...
	return JB_Syntax_Msg$msgstr(Type, self, Name);
	return nil;
}
...
__lib__ Message* jb_msg_expect(Message* self, Syntax Type, JB_String* name, Message* ErrPlace) {
	if (self) {
		...
		JB_Msg_AccessErr$syntaxstr(ErrPlace, Type, name);
	}
	return nil;
}
...
__lib__ Syntax jb_syntax(JB_String* name) {
	SyntaxObj* _tmP = JB_Incr(((SyntaxObj*)JB_Dict_SyntaxAccess$str(JB__SyxDict_, name)));
	if ((!_tmP)) {
		JB_Decr(_tmP);
		return nil;
	}
...
// 3490376491474110898 6104727632359725163