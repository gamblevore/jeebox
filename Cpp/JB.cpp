...
int JB_Constants__Init_() {
	{
		...
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_227, JB_str_51));
		...
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_50, JB_str_231));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_21, JB_str_232));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_53, JB_str_233));
		...
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_50, JB_str_231));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_53, JB_str_233));
...
JB_String* JB_Constants__Test() {
	...
	JB_Decr(fs);
	if ((!(({
		JB_String* _tmp388 = JB_Incr(JB_Str_Unescape(JB_Str_Escape(str)));
		bool _tmp387 = JB_Str_SyntaxEquals(_tmp388, str, false);
		JB_Decr(_tmp388);
		_tmp387;
	})))) {
	...
	if (JB_Str_Exists(list_input)) {
		...
		({
			Message* _tmp390 = JB_Incr(JB_Msg_Copy(root, false));
			bool _tmp389 = JB_Msg_TreeCompare(root, _tmp390);
			JB_Decr(_tmp390);
			_tmp389;
		});
		Message* list = JB_Incr(({
			Message* _tmp392 = JB_Incr(JB_Str_Parse(list_input, nil));
			Message* _tmp391 = JB_Incr(JB_Msg_ParseAST(_tmp392));
			JB_Decr(_tmp392);
			JB_SafeDecr(_tmp391);
			_tmp391;
		}));
...
void JB_Tk__ErrorEvent2(int Start, int ExpectedBits, int RealBits) {
	...
	if (RealBits == kJB__Tk_EndOfLine) {
		...
		if (((bool)(RealBits & kJB__Tk_IllegalChar))) {
			...
			JB_String* _tmp1 = JB_Incr(({
				JB_String* _tmp380 = JB_Incr(JB_Str_Range(JB__Tk_Data, Start, N));
				JB_String* _tmp379 = JB_Incr(JB_Str_Escape(_tmp380));
				JB_Decr(_tmp380);
				JB_SafeDecr(_tmp379);
				_tmp379;
			}));
	...
	({
		JB_String* _tmp382 = JB_Incr(JB_FS_SyntaxCastString(Err));
		Message* _tmp381 = JB_Incr(JB_Tk__ErrorAdd(_tmp382, Start, false));
		JB_Decr(_tmp382);
		JB_SafeDecr(_tmp381);
		_tmp381;
	});
...
void JB_Tk__ErrorLetter(int Start) {
	JB_FreeIfDead(JB_Tk__ErrorAdd(JB_str_143, Start, false));
}
...
Message* JB_Tk__fBracket(int Start) {
	...
	if (JB_Tk__ParseLoop(Result, JB_str_130)) {
...
Message* JB_Tk__fChr(int Start) {
	...
	JB_String* name = JB_Incr(({
		JB_String* _tmp363 = JB_Incr(JB_Str_Range(JB__Tk_Data, Start + 1, After));
		JB_String* _tmp362 = JB_Incr(JB_Str_Unescape(_tmp363));
		JB_Decr(_tmp363);
		JB_SafeDecr(_tmp362);
		_tmp362;
	}));
...
Message* JB_Tk__fComment(int Start) {
	...
	while (true) {
		...
		if ((!JB_int_Found(i))) {
			JB_Decr(d);
			return JB_Tk__ErrorAdd(JB_str_132, Start, false);
...
Message* JB_Tk__fMsgList(int Start) {
	...
	Message* Emb = JB_Incr(JB_Tk__NewParent0(Result, JB_SyxEmb, Start));
	int ops = kJB__Tk_OppSyx | (kJB__Tk_OppTemporal | kJB__Tk_OppYoda);
	Message* _tmp = JB_Incr(JB_Tk__ProcessThing(ops, true));
	JB_Tree_SyntaxAppend(Emb, _tmp);
	JB_Decr(_tmp);
	Message* _tmp1 = JB_Incr(JB_Tk__ProcessThing(ops, false));
	JB_Tree_SyntaxAppend(Emb, _tmp1);
	JB_Decr(_tmp1);
	JB_Decr(Emb);
	JB_Tk__ConsumeLines(nil, nil);
	while ((!JB_Tk__WillEnd())) {
		Message* item = JB_Incr(JB_Tk__ProcessThing(kJB__Tk_Opp, false));
		JB_Tk__ConsumeLines(Result, item);
		JB_Decr(item);
	};
	JB_Tk__ExpectEndChar(JB_str_128, true);
	JB_SafeDecr(Result);
	return Result;
}
...
Message* JB_Tk__fString2(int Start) {
	...
	while (true) {
		...
		if (C0 == '{') {
			StrPos = JB_Tk__EmbeddedCode(JB_str_134, result, JB_Syntax_Parseflags(JB_SyxList));
		} else {
		...
		if (StrPos > After) {
			JB_Decr(result);
			return JB_Tk__ErrorAdd(JB_str_136, NewStrPos, false);
		}
	};
	(JB_Tk__NextStartSet(After + 1));
...
Message* JB_Tk__fSyntacticComment(int Start) {
	...
	JB_Tk__EmbeddedArg(JB_str_137, Result, JB_SyxList);
	JB_SafeDecr(Result);
	return Result;
}
...
Message* JB_Tk__GetFuncAfter(Message* input) {
	...
	while (true) {
		...
		if ((!JB_Msg_SyxOppEquals(d, JB_SyxName, false))) {
			...
			JB_FreeIfDead(JB_Tk__ErrorAdd(JB_str_140, JB_Tk__NextStart(), false));
		} else {
...
Message* JB_Tk__MakeRel(Message* first, int Bits) {
	...
	JB_Tree_SyntaxAppend(Rel, first);
	JB_Tree_SyntaxAppend(Rel, opp);
	if (JB_Tk__EatString(JB_str_141)) {
		Rel->Func = JB_SyxERel;
	}
...
int JB_Tk__MessageErrorSub(FastString* fs, int num, int ButFound) {
	if ((!JB__Tk_ErrorNames)) {
		...
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_Temporal, JB_str_149));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_TemporalSyx, JB_str_150));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_TemporalWord, JB_str_151));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_SyntacticComment, JB_str_146));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_EndOfLine, JB_str_145));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_colon, JB_str_152));
		(JB_Dict_a4(JB__Tk_ErrorNames, kJB__Tk_OppSyx, JB_str_153));
...
int JB_Init_() {
	{
		JB_SetRef(JB_str_320, JB_Str3("Binary", 6));
		JB_SetRef(JB_str_319, JB_Str3("bin", 3));
		JB_SetRef(JB_str_318, JB_Str3("Message_List", 12));
		JB_SetRef(JB_str_317, JB_Str3("msg", 3));
		JB_SetRef(JB_str_316, JB_Str3("Question", 8));
		JB_SetRef(JB_str_315, JB_Str3("ask", 3));
		JB_SetRef(JB_str_314, JB_Str3("yopp", 4));
		JB_SetRef(JB_str_313, JB_Str3("Yoda_Relationship", 17));
		JB_SetRef(JB_str_312, JB_Str3("yoda", 4));
		JB_SetRef(JB_str_311, JB_Str3("Typecast", 8));
		JB_SetRef(JB_str_310, JB_Str3("type", 4));
		JB_SetRef(JB_str_309, JB_Str3("Conjugate", 9));
		JB_SetRef(JB_str_308, JB_Str3("cnj", 3));
		JB_SetRef(JB_str_307, JB_Str3("Syntactic_Conjugate", 19));
		JB_SetRef(JB_str_306, JB_Str3("scnj", 4));
		JB_SetRef(JB_str_305, JB_Str3("Temporal_Reference", 18));
		JB_SetRef(JB_str_304, JB_Str3("ana", 3));
		JB_SetRef(JB_str_303, JB_Str3("Statement", 9));
		JB_SetRef(JB_str_302, JB_Str3("tmp", 3));
		JB_SetRef(JB_str_301, JB_Str3("Thing", 5));
		JB_SetRef(JB_str_300, JB_Str3("thg", 3));
		JB_SetRef(JB_str_299, JB_Str3("Syntactic_Thing", 15));
		JB_SetRef(JB_str_298, JB_Str3("sthg", 4));
		JB_SetRef(JB_str_297, JB_Str3("String", 6));
		JB_SetRef(JB_str_296, JB_Str3("str", 3));
		JB_SetRef(JB_str_295, JB_Str3("Shebang", 7));
		JB_SetRef(JB_str_294, JB_Str3("sheb", 4));
		JB_SetRef(JB_str_293, JB_Str3("Syntactic_Comment", 17));
		JB_SetRef(JB_str_292, JB_Str3("scom", 4));
		JB_SetRef(JB_str_291, JB_Str3("Self_Dot", 8));
		JB_SetRef(JB_str_290, JB_Str3("sdot", 4));
		JB_SetRef(JB_str_289, JB_Str3("Temporal_Relationship", 21));
		JB_SetRef(JB_str_288, JB_Str3("trel", 4));
		JB_SetRef(JB_str_287, JB_Str3("Relationship", 12));
		JB_SetRef(JB_str_286, JB_Str3("rel", 3));
		JB_SetRef(JB_str_285, JB_Str3("Syntactic_Operator", 18));
		JB_SetRef(JB_str_284, JB_Str3("sopp", 4));
		JB_SetRef(JB_str_283, JB_Str3("Operator", 8));
		JB_SetRef(JB_str_282, JB_Str3("opp", 3));
		JB_SetRef(JB_str_281, JB_Str3("Operator_As_Thing", 17));
		JB_SetRef(JB_str_280, JB_Str3("oat", 3));
		JB_SetRef(JB_str_279, JB_Str3("unit", 4));
		JB_SetRef(JB_str_278, JB_Str3("Number", 6));
		JB_SetRef(JB_str_277, JB_Str3("num", 3));
		JB_SetRef(JB_str_276, JB_Str3("name", 4));
		JB_SetRef(JB_str_275, JB_Str3("item", 4));
		JB_SetRef(JB_str_274, JB_Str3("Function", 8));
		JB_SetRef(JB_str_273, JB_Str3("func", 4));
		JB_SetRef(JB_str_272, JB_Str3("Equals_Relationship", 19));
		JB_SetRef(JB_str_271, JB_Str3("erel", 4));
		JB_SetRef(JB_str_270, JB_Str3("Embedded_Value", 14));
		JB_SetRef(JB_str_269, JB_Str3("emb", 3));
		JB_SetRef(JB_str_268, JB_Str3("Dot", 3));
		JB_SetRef(JB_str_267, JB_Str3("dot", 3));
		JB_SetRef(JB_str_266, JB_Str3("Declaration", 11));
		JB_SetRef(JB_str_265, JB_Str3("decl", 4));
		JB_SetRef(JB_str_264, JB_Str3("Character", 9));
		JB_SetRef(JB_str_263, JB_Str3("Before_Relationship", 19));
		JB_SetRef(JB_str_262, JB_Str3("brel", 4));
		JB_SetRef(JB_str_261, JB_Str3("Bracket", 7));
		JB_SetRef(JB_str_260, JB_Str3("bra", 3));
		JB_SetRef(JB_str_259, JB_Str3("Super_String", 12));
		JB_SetRef(JB_str_258, JB_Str3("sstr", 4));
		JB_SetRef(JB_str_257, JB_Str3("Backtick", 8));
		JB_SetRef(JB_str_256, JB_Str3("back", 4));
		JB_SetRef(JB_str_255, JB_Str3("Array", 5));
		JB_SetRef(JB_str_254, JB_Str3("arr", 3));
		JB_SetRef(JB_str_253, JB_Str3("After_Relationship", 18));
		JB_SetRef(JB_str_252, JB_Str3("arel", 4));
		JB_SetRef(JB_str_251, JB_Str3("Adjective", 9));
		JB_SetRef(JB_str_250, JB_Str3("adj", 3));
		JB_SetRef(JB_str_249, JB_Str3("Access", 6));
		JB_SetRef(JB_str_248, JB_Str3("acc", 3));
		...
		JB_SetRef(JB_str_153, JB_Str3("syntactic-operator", 18));
		JB_SetRef(JB_str_152, JB_Str3("':'", 3));
		JB_SetRef(JB_str_151, JB_Str3("temporal-word", 13));
		JB_SetRef(JB_str_150, JB_Str3("declaration", 11));
		JB_SetRef(JB_str_149, JB_Str3("temporal", 8));
		JB_SetRef(JB_str_148, JB_Str3("Found illegal-character ", 24));
		JB_SetRef(JB_str_147, JB_Str3("end-of-data", 11));
		JB_SetRef(JB_str_146, JB_Str3("syntactic-comment", 17));
		JB_SetRef(JB_str_145, JB_Str3("end-of-line", 11));
		JB_SetRef(JB_str_144, JB_Str3("Unexpected ", 11));
		JB_SetRef(JB_str_143, JB_Str3("Expected a letter (aA-zZ, '_', etc).", 36));
		JB_SetRef(JB_str_142, JB_Str3("Can’t use temporals like this.", 32));
		JB_SetRef(JB_str_141, JB_Str3("=", 1));
		JB_SetRef(JB_str_140, JB_Str3("Must be a proper name (a-z or letters)", 38));
		JB_SetRef(JB_str_139, JB_Str3("' not allowed.", 14));
		JB_SetRef(JB_str_138, JB_Str3("Nested '", 8));
		JB_SetRef(JB_str_137, JB_Str3("#/", 2));
		JB_SetRef(JB_str_136, JB_Str3("Embedded expression didn't close.", 33));
		JB_SetRef(JB_str_135, JB_Str3("String didn't close!", 20));
		JB_SetRef(JB_str_134, JB_Str3("}", 1));
		JB_SetRef(JB_str_133, JB_Str3("Can’t have '=' after this operator.", 37));
		JB_SetRef(JB_str_132, JB_Str3("Comment did not close.", 22));
		JB_SetRef(JB_str_131, JB_Str3("Char didn't close!", 18));
		JB_SetRef(JB_str_130, JB_Str3(")", 1));
		JB_SetRef(JB_str_129, JB_Str3("String did not close.", 21));
		JB_SetRef(JB_str_128, JB_Str3("»", 2));
		...
		JB_SyxAcc = JB_Syntax__StdNew(JB_Msg_Acc__, JB_str_248, JB_str_249);
		JB_SyxAdj = JB_Syntax__StdNew(JB_Msg_Adj__, JB_str_250, JB_str_251);
		JB_SyxARel = JB_Syntax__StdNew(JB_Msg_ARel__, JB_str_252, JB_str_253);
		JB_SyxArr = JB_Syntax__StdNew(JB_Msg_Arr__, JB_str_254, JB_str_255);
		JB_SyxBack = JB_Syntax__StdNew(JB_Msg_Back__, JB_str_256, JB_str_257);
		JB_SyxSStr = JB_Syntax__StdNew(JB_Msg_SStr__, JB_str_258, JB_str_259);
		JB_SyxBra = JB_Syntax__StdNew(JB_Msg_Bra__, JB_str_260, JB_str_261);
		JB_SyxBRel = JB_Syntax__StdNew(JB_Msg_BRel__, JB_str_262, JB_str_263);
		JB_SyxChar = JB_Syntax__StdNew(JB_Msg_Char__, JB_str_178, JB_str_264);
		JB_SyxDecl = JB_Syntax__StdNew(JB_Msg_Decl__, JB_str_265, JB_str_266);
		JB_SyxDot = JB_Syntax__StdNew(JB_Msg_Dot__, JB_str_267, JB_str_268);
		JB_SyxEmb = JB_Syntax__StdNew(JB_Msg_Emb__, JB_str_269, JB_str_270);
		JB_SyxERel = JB_Syntax__StdNew(JB_Msg_ERel__, JB_str_271, JB_str_272);
		JB_SyxFunc = JB_Syntax__StdNew(JB_Msg_Func__, JB_str_273, JB_str_274);
		JB_SyxItem = JB_Syntax__StdNew(JB_Msg_Item__, JB_str_275, JB_str_0);
		JB_SyxList = JB_Syntax__StdNew(JB_Msg_List__, JB_str_167, JB_str_0);
		JB_SyxName = JB_Syntax__StdNew(JB_Msg_Name__, JB_str_276, JB_str_0);
		JB_SyxNum = JB_Syntax__StdNew(JB_Msg_Num__, JB_str_277, JB_str_278);
		JB_SyxUnit = JB_Syntax__StdNew(JB_Msg_Unit__, JB_str_279, JB_str_0);
		JB_SyxOat = JB_Syntax__StdNew(JB_Msg_Oat__, JB_str_280, JB_str_281);
		JB_SyxOpp = JB_Syntax__StdNew(JB_Msg_Opp__, JB_str_282, JB_str_283);
		JB_SyxSOpp = JB_Syntax__StdNew(JB_Msg_SOpp__, JB_str_284, JB_str_285);
		JB_SyxRel = JB_Syntax__StdNew(JB_Msg_Rel__, JB_str_286, JB_str_287);
		JB_SyxTRel = JB_Syntax__StdNew(JB_Msg_TRel__, JB_str_288, JB_str_289);
		JB_SyxSDot = JB_Syntax__StdNew(JB_Msg_SDot__, JB_str_290, JB_str_291);
		JB_SyxSCom = JB_Syntax__StdNew(JB_Msg_SCom__, JB_str_292, JB_str_293);
		JB_SyxSheb = JB_Syntax__StdNew(JB_Msg_Sheb__, JB_str_294, JB_str_295);
		JB_SyxStr = JB_Syntax__StdNew(JB_Msg_Str__, JB_str_296, JB_str_297);
		JB_SyxSThg = JB_Syntax__StdNew(JB_Msg_SThg__, JB_str_298, JB_str_299);
		JB_SyxThg = JB_Syntax__StdNew(JB_Msg_Thg__, JB_str_300, JB_str_301);
		JB_SyxTmp = JB_Syntax__StdNew(JB_Msg_Tmp__, JB_str_302, JB_str_303);
		JB_SyxAna = JB_Syntax__StdNew(JB_Msg_Ana__, JB_str_304, JB_str_305);
		JB_SyxSCnj = JB_Syntax__StdNew(JB_Msg_SCnj__, JB_str_306, JB_str_307);
		JB_SyxCnj = JB_Syntax__StdNew(JB_Msg_Cnj__, JB_str_308, JB_str_309);
		JB_SyxType = JB_Syntax__StdNew(JB_Msg_Type__, JB_str_310, JB_str_311);
		JB_SyxYoda = JB_Syntax__StdNew(JB_Msg_Yoda__, JB_str_312, JB_str_313);
		JB_SyxYopp = JB_Syntax__StdNew(JB_Msg_Yopp__, JB_str_314, JB_str_0);
		JB_SyxAsk = JB_Syntax__StdNew(JB_Msg_Ask__, JB_str_315, JB_str_316);
		JB_SyxMsg = JB_Syntax__StdNew(JB_Msg_Msg__, JB_str_317, JB_str_318);
		JB_SyxBin = JB_Syntax__StdNew(JB_Msg_Bin__, JB_str_319, JB_str_320);
	};
	//// JB_API;
	JB_API__Init_();
	//// Jeebox;
...
Message* JB_Str_ParseAs(JB_String* self, JB_String* name) {
	if ((!JB_Str_Exists(name)) or (JB_Str_SyntaxEquals(name, JB_str_166, true))) {
		...
		return ({
			Message* _tmp386 = JB_Incr(JB_Str_Parse(self, nil));
...
void JB_Msg_Msg__(Message* self, FastString* fs) {
	...
	while (ch) {
		if (ch == first) {
			...
			JB_FS_AppendString(fs, JB_str_43);
			JB_FS_AppendIndent(fs);
		}
		JB_FS_AppendMsg(fs, ch);
		JB_SetRef(ch, ((Message*)JB_Ring_NextSib(ch)));
