...
int JB_Constants__Init_() {
	{
		...
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_163, JB_str_164));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_15, JB_str_67));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_165, JB_str_166));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_104, JB_str_167));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_14, JB_str_168));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_169, JB_str_170));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_171, JB_str_172));
		...
		(JB_Dict_ValueSet(JB__Constants_UnEscapeStr, JB_str_164, ((JB_String*)nil)));
		...
		JB_SetRef(JB__Constants_CSWordMiddle, JB_Str_Charset(JB_str_175, true));
		JB_SetRef(JB__Constants_CSWordStart, JB_Str_Charset(JB_str_176, true));
		JB_SetRef(JB__Constants_CSNum, JB__Constants_CSWordMiddle);
		JB_SetRef(JB__Constants_CSLine, JB_Str_Charset(JB_str_177, false));
		JB_SetRef(JB__Constants_CSAfterTemporal, JB_Str_Charset(JB_str_178, false));
		JB_SetRef(JB__Constants_CSLineBlack, JB_Str_Charset(JB_str_179, false));
		JB_SetRef(JB__Constants_CSNumUnit, JB_Str_Charset(JB_str_180, true));
	};
	return 0;
}
...
JB_String* JB_Constants__Test() {
	...
	JB_Incr(str);
	JB_Decr(fs);
	JB_String* _tmp322 = JB_Str_Unescape(JB_Str_Escape(str));
	JB_Incr(_tmp322);
	if ((!(JB_Str_SyntaxEquals(_tmp322, str, false)))) {
	...
	if (JB_Str_Exists(list_input)) {
		...
		Message* _tmp323 = JB_Msg_Copy(root, false);
		JB_Incr(_tmp323);
		JB_Msg_TreeCompare(root, _tmp323);
		JB_Decr(_tmp323);
		Message* _tmp324 = JB_Str_Parse(list_input);
		JB_Incr(_tmp324);
		Message* list = JB_Msg_ParseReadable(_tmp324);
		JB_Incr(list);
		JB_Decr(_tmp324);
		JB_Msg_TreeCompare(root, list);
		JB_Decr(list);
		JB_String* _tmp325 = JB_Msg_Render(root, nil);
		JB_Incr(_tmp325);
		JB_Msg_Test(root, _tmp325, JB_str_146);
		JB_Decr(_tmp325);
		JB_String* _tmp326 = JB_Msg_MakeReadable(root, nil);
		JB_Incr(_tmp326);
		JB_Msg_Test(root, _tmp326, JB_str_147);
		JB_Decr(_tmp326);
		JB_Decr(root);
	} else {
		Message* root = JB_Str_Parse(natural_input);
...
int JB_Tk__EmbeddedArg(JB_String* close, Message* result, Syntax* Mode) {
	...
	JB_Incr(arg);
	int _tmp306 = JB_Tk__EmbeddedCode(close, arg, JB_Fn_Parseflags(Mode));
	JB_Decr(arg);
	return _tmp306;
}
...
int JB_Tk__EmbeddedCode(JB_String* close, Message* dest, int Mode) {
	...
	JB_Incr(_tmp307);
	int result = JB_Str_FindStr(_tmp307, JB_Tk__NextStart(), close);
	JB_Decr(_tmp307);
...
void JB_Tk__ErrorEvent2(int Start, int ExpectedBits, int RealBits) {
	...
	if (RealBits == kJB__Tk_EndOfLine) {
		...
		if (((bool)(RealBits & kJB__Tk_IllegalChar))) {
			...
			JB_Incr(_tmp315);
			JB_FS_AppendHex(Err, JB_Str_SyntaxAccess(_tmp315, Start), 2);
			JB_Decr(_tmp315);
		} else {
			JB_FS_AppendByte(Err, '\'');
			int N = JB_int_OperatorMin(JB_Tk__NextStart(), (Start + 100));
			JB_String* _tmp316 = JB_Str_Escape(JB_Str_AccessRange(JB_Tk__Data(), Start, N));
			JB_Incr(_tmp316);
			JB_FS_AppendString(Err, _tmp316);
			JB_Decr(_tmp316);
			JB_FS_AppendByte(Err, '\'');
		}
	}
	JB_String* _tmp317 = JB_FS_SyntaxCastString(Err);
	JB_Incr(_tmp317);
	JB_Decr(Err);
	JB_FreeIfDead(JB_Tk__ErrorAdd(_tmp317, Start, false));
	JB_Decr(_tmp317);
}
...
Message* JB_Tk__fAdjectiveOp(int Start) {
	...
	JB_Incr(result);
	Message* _tmp296 = JB_Tk__Process(kJB__Tk_OppChain, kJB__Tk_Expect);
	JB_Incr(_tmp296);
...
Message* JB_Tk__fAtNamedExp(int Start) {
	...
	JB_Incr(result);
	Message* _tmp287 = JB_Tk__ProcessThing(0, kJB__Tk_Expect);
	JB_Incr(_tmp287);
	JB_Tree_SyntaxAppend(result, _tmp287);
	JB_Decr(_tmp287);
	JB_SafeDecr(result);
	return result;
}
...
Message* JB_Tk__fBeforeRelMinus(int Start) {
	...
	JB_Incr(_tmp289);
	byte c = JB_Str_SyntaxAccess(_tmp289, Start + 1);
	JB_Decr(_tmp289);
	if (JB_byte_IsNum(c)) {
...
Message* JB_Tk__fChr(int Start) {
	...
	JB_Incr(_tmp290);
	int After = JB_Str_JBFind(_tmp290, JB_Tk__NextStart(), kJB__int_max, '\'');
	JB_Decr(_tmp290);
	if ((!JB_int_Found(After))) {
		return JB_Tk__ErrorAdd(JB_str_111, JB_Tk__NextStart(), false);
	}
	(JB_Tk__NextStartSet(After + 1));
	JB_String* _tmp291 = JB_Str_AccessRange(JB_Tk__Data(), Start + 1, After);
	JB_Incr(_tmp291);
	JB_String* name = JB_Str_Unescape(_tmp291);
	JB_Incr(name);
	JB_Decr(_tmp291);
	Message* _tmp292 = JB_Tk__NewParentName(nil, JB_SyxChar, Start, name);
	JB_Incr(_tmp292);
	JB_Decr(name);
	JB_SafeDecr(_tmp292);
	return _tmp292;
...
Message* JB_Tk__fCommentLine(int Start) {
	JB_String* _tmp293 = JB_Tk__Data();
	JB_Incr(_tmp293);
	(JB_Tk__NextStartSet(JB_Str_InCharSet(_tmp293, JB_Tk__NextStart(), kJB__int_max, JB__Constants_CSLine)));
	JB_Decr(_tmp293);
	return JB_Tk__Avoid();
}
...
Message* JB_Tk__fOpAsThing(int Start) {
	...
	JB_Incr(Result);
	Message* _tmp294 = JB_Tk__Process(kJB__Tk_Opp, 0);
	JB_Incr(_tmp294);
	JB_Tree_SyntaxAppend(Result, _tmp294);
	JB_Decr(_tmp294);
	Message* _tmp295 = JB_Tk__ProcessThing(kJB__Tk_Opp, 0);
	JB_Incr(_tmp295);
...
Message* JB_Tk__fOppBracket(int Start) {
	...
	if (opp2) {
		...
		Message* _tmp298 = JB_Tk__Process(kJB__Tk_OppChain, 0);
		JB_Incr(_tmp298);
		JB_Tree_SyntaxAppend(rel, _tmp298);
		JB_Decr(_tmp298);
		JB_Decr(rel);
	} else {
	...
	Message* _tmp299 = JB_Tk__OppChain(result);
	JB_Incr(_tmp299);
	JB_Decr(result);
	JB_SafeDecr(_tmp299);
	return _tmp299;
...
Message* JB_Tk__fOppWord(int Start) {
	...
	JB_Incr(opp);
	Message* _tmp297 = JB_Tk__OppChain(opp);
	JB_Incr(_tmp297);
	JB_Decr(opp);
	JB_SafeDecr(_tmp297);
	return _tmp297;
...
Message* JB_Tk__fSDot(int Start) {
	if (Start > 0) {
		...
		JB_Incr(_tmp300);
		if (JB_BM_HasChar(JB__Constants_CSWordMiddle, JB_Str_SyntaxAccess(_tmp300, Start - 1))) {
			JB_Decr(_tmp300);
			return nil;
		}
		JB_Decr(_tmp300);
...
Message* JB_Tk__fShebang(int Start) {
	...
	JB_Incr(_tmp301);
	int S2 = JB_Str_InCharSet(_tmp301, S, kJB__int_max, JB__Constants_CSLine);
	JB_Decr(_tmp301);
...
Message* JB_Tk__fString(int Start) {
	...
	while (true) {
		...
		JB_Incr(_tmp303);
		int NewStrPos = JB_Str_InStr(_tmp303, JB_str_115, StrPos, After, false);
		JB_Decr(_tmp303);
		...
		Message* _tmp304 = ((Message*)JB_Tree_Last(result));
		JB_Incr(_tmp304);
		if (_tmp304 == OldLast) {
			JB_Decr(result);
			JB_Decr(OldLast);
...
Message* JB_Tk__fStrSub(int Start, JB_String* Ender, Syntax* syx) {
	...
	JB_Incr(_tmp288);
	int After = JB_Str_FindStr(_tmp288, JB_Tk__NextStart(), Ender);
	JB_Decr(_tmp288);
	if ((!JB_int_Found(After))) {
...
Message* JB_Tk__fTemporalAsThing(int Start) {
	...
	JB_Incr(result);
	Message* _tmp305 = JB_Tk__Process(kJB__Tk_Temporal, kJB__Tk_Expect);
	JB_Incr(_tmp305);
...
void JB_Tk__Init() {
	...
	JB_Decr(_tmp281);
	TokHan* _tmp282 = JB_TH_Link(JB_Tk__Handler(kJB__Tk_ThingSyx, ((TokenHandler_fp)JB_Tk__fAtName)), kJB__Tk_FuncAfterNormal, ((TokenHandler_fp)JB_Tk__fAtNamedExp));
	JB_Incr(_tmp282);
...
Message* JB_Tk__MakeRel(Message* first, int Bits) {
	...
	JB_Decr(opp);
	Message* _tmp308 = JB_Tk__ProcessThing(Bits, Mode);
	JB_Incr(_tmp308);
...
int JB_Tk__MessageErrorSub(FastString* fs, int num, int ButFound) {
	...
	if (BestValue) {
		JB_String* _tmp318 = ((JB_String*)JB_Dict_Value(JB__Tk_ErrorNames, JB_int_DictName(BestValue)));
		JB_Incr(_tmp318);
		JB_FS_AppendString(fs, _tmp318);
		JB_Decr(_tmp318);
...
Message* JB_Tk__ReRoute(Message* cnj, Message* output) {
	...
	if ((JB_Tree_HasChildCount(Result, 2)) and JB_Tk__Allow(kJB__Tk_EndOfLine)) {
		Message* _tmp310 = JB_Tk__ProcessThing(kJB__Tk_Opp, 0);
		JB_Incr(_tmp310);
...
void JB_Tk__Tokens(Array* arr, int bits, ParseHandler func) {
	...
	JB_Incr(_tmp311);
	JB_Tk__TokensHandler(arr, _tmp311);
	JB_Decr(_tmp311);
}
...
void JB_Tk__Tokenz(JB_String* s, int bits, ParseHandler func) {
	if (JB_Str_OperatorContains(s, JB_str_33)) {
		Array* _tmp312 = JB_Str_Split(s, JB_str_33);
		JB_Incr(_tmp312);
		JB_Tk__Tokens(_tmp312, bits, func);
		JB_Decr(_tmp312);
	} else {
		TokHan* _tmp313 = JB_Tk__Handler(bits, ((TokenHandler_fp)func));
		JB_Incr(_tmp313);
		(JB_Tk__TokenSet(s, _tmp313));
		JB_Decr(_tmp313);
	}
}
...
int JB_Init_() {
	{
		JB_SetRef(JB_str_253, JB_Str3("Binary", 6));
		JB_SetRef(JB_str_252, JB_Str3("bin", 3));
		JB_SetRef(JB_str_251, JB_Str3("unit", 4));
		JB_SetRef(JB_str_250, JB_Str3("Typecast", 8));
		JB_SetRef(JB_str_249, JB_Str3("type", 4));
		JB_SetRef(JB_str_248, JB_Str3("Conjugate", 9));
		JB_SetRef(JB_str_247, JB_Str3("cnj", 3));
		JB_SetRef(JB_str_246, JB_Str3("Syntactic_Conjugate", 19));
		JB_SetRef(JB_str_245, JB_Str3("scnj", 4));
		JB_SetRef(JB_str_244, JB_Str3("Temporal_Reference", 18));
		JB_SetRef(JB_str_243, JB_Str3("ana", 3));
		JB_SetRef(JB_str_242, JB_Str3("Temporal_Statement", 18));
		JB_SetRef(JB_str_241, JB_Str3("tmp", 3));
		JB_SetRef(JB_str_240, JB_Str3("Temporal_Chain", 14));
		JB_SetRef(JB_str_239, JB_Str3("chn", 3));
		JB_SetRef(JB_str_238, JB_Str3("Thing", 5));
		JB_SetRef(JB_str_237, JB_Str3("thg", 3));
		JB_SetRef(JB_str_236, JB_Str3("Syntactic_Thing", 15));
		JB_SetRef(JB_str_235, JB_Str3("sthg", 4));
		JB_SetRef(JB_str_234, JB_Str3("String", 6));
		JB_SetRef(JB_str_233, JB_Str3("str", 3));
		JB_SetRef(JB_str_232, JB_Str3("Shebang", 7));
		JB_SetRef(JB_str_231, JB_Str3("sheb", 4));
		JB_SetRef(JB_str_230, JB_Str3("Syntactic_Comment", 17));
		JB_SetRef(JB_str_229, JB_Str3("scom", 4));
		JB_SetRef(JB_str_228, JB_Str3("Self_Dot", 8));
		JB_SetRef(JB_str_227, JB_Str3("sdot", 4));
		JB_SetRef(JB_str_226, JB_Str3("Temporal_Relationship", 21));
		JB_SetRef(JB_str_225, JB_Str3("trel", 4));
		JB_SetRef(JB_str_224, JB_Str3("Relationship", 12));
		JB_SetRef(JB_str_223, JB_Str3("rel", 3));
		JB_SetRef(JB_str_222, JB_Str3("Syntactic_Operator", 18));
		JB_SetRef(JB_str_221, JB_Str3("sopp", 4));
		JB_SetRef(JB_str_220, JB_Str3("Operator", 8));
		JB_SetRef(JB_str_219, JB_Str3("opp", 3));
		JB_SetRef(JB_str_218, JB_Str3("Operator_As_Thing", 17));
		JB_SetRef(JB_str_217, JB_Str3("oat", 3));
		JB_SetRef(JB_str_216, JB_Str3("Number", 6));
		JB_SetRef(JB_str_215, JB_Str3("num", 3));
		JB_SetRef(JB_str_214, JB_Str3("name", 4));
		JB_SetRef(JB_str_213, JB_Str3("Parameters", 10));
		JB_SetRef(JB_str_212, JB_Str3("prm", 3));
		JB_SetRef(JB_str_211, JB_Str3("item", 4));
		JB_SetRef(JB_str_210, JB_Str3("Function", 8));
		JB_SetRef(JB_str_209, JB_Str3("func", 4));
		JB_SetRef(JB_str_208, JB_Str3("Equals_Relationship", 19));
		JB_SetRef(JB_str_207, JB_Str3("erel", 4));
		JB_SetRef(JB_str_206, JB_Str3("Embedded_Value", 14));
		JB_SetRef(JB_str_205, JB_Str3("emb", 3));
		JB_SetRef(JB_str_204, JB_Str3("Dot", 3));
		JB_SetRef(JB_str_203, JB_Str3("dot", 3));
		JB_SetRef(JB_str_202, JB_Str3("Declaration", 11));
		JB_SetRef(JB_str_201, JB_Str3("decl", 4));
		JB_SetRef(JB_str_200, JB_Str3("Character", 9));
		JB_SetRef(JB_str_199, JB_Str3("char", 4));
		JB_SetRef(JB_str_198, JB_Str3("Before_Relationship", 19));
		JB_SetRef(JB_str_197, JB_Str3("brel", 4));
		JB_SetRef(JB_str_196, JB_Str3("Bracket", 7));
		JB_SetRef(JB_str_195, JB_Str3("bra", 3));
		JB_SetRef(JB_str_194, JB_Str3("Super_String", 12));
		JB_SetRef(JB_str_193, JB_Str3("sstr", 4));
		JB_SetRef(JB_str_192, JB_Str3("Backtick", 8));
		JB_SetRef(JB_str_191, JB_Str3("back", 4));
		JB_SetRef(JB_str_190, JB_Str3("Array", 5));
		JB_SetRef(JB_str_189, JB_Str3("arr", 3));
		JB_SetRef(JB_str_188, JB_Str3("After_Relationship", 18));
		JB_SetRef(JB_str_187, JB_Str3("arel", 4));
		JB_SetRef(JB_str_186, JB_Str3("Adjective", 9));
		JB_SetRef(JB_str_185, JB_Str3("adj", 3));
		JB_SetRef(JB_str_184, JB_Str3("Access", 6));
		JB_SetRef(JB_str_183, JB_Str3("acc", 3));
		JB_SetRef(JB_str_182, JB_Str3("Argument", 8));
		JB_SetRef(JB_str_181, JB_Str3("arg", 3));
		JB_SetRef(JB_str_180, JB_Str3("AZazÄø¬Ù", 8));
		JB_SetRef(JB_str_179, JB_Str3("\n\r,", 3));
		JB_SetRef(JB_str_178, JB_Str3(" ,:\t\n\r\\", 7));
		JB_SetRef(JB_str_177, JB_Str3("\n\r", 2));
		JB_SetRef(JB_str_176, JB_Str3("AZaz__Äø¬Ù", 10));
		JB_SetRef(JB_str_175, JB_Str3("AZaz09__Äø¬Ù", 12));
		JB_SetRef(JB_str_174, JB_Str3("\\&#", 3));
		JB_SetRef(JB_str_173, JB_Str3("&#", 2));
		JB_SetRef(JB_str_172, JB_Str3("\\\\", 2));
		JB_SetRef(JB_str_171, JB_Str3("\\", 1));
		JB_SetRef(JB_str_170, JB_Str3("\\t", 2));
		JB_SetRef(JB_str_169, JB_Str3("\t", 1));
		JB_SetRef(JB_str_168, JB_Str3("\\'", 2));
		JB_SetRef(JB_str_167, JB_Str3("\\\"", 2));
		JB_SetRef(JB_str_166, JB_Str3("\\r", 2));
		JB_SetRef(JB_str_165, JB_Str3("\r", 1));
		JB_SetRef(JB_str_164, JB_Str3("\\0", 2));
		JB_SetRef(JB_str_163, JB_Str3("\0", 1));
		JB_SetRef(JB_str_162, JB_Str3("[0;1;33m", 9));
		JB_SetRef(JB_str_161, JB_Str3("[0;1;32m", 9));
		JB_SetRef(JB_str_160, JB_Str3("[0;1;31m", 9));
		JB_SetRef(JB_str_159, JB_Str3("[0;1m", 6));
		JB_SetRef(JB_str_158, JB_Str3("[0m", 4));
		...
		JB_SetRef(JB_str_122, JB_Str3("Must be a proper name (a-z or letters)", 38));
		...
		JB_SetRef(JB_SyxArg, JB_Fn__StdNew(JB_Msg_Arg__, 0, JB_str_181, JB_str_182));
		JB_SetRef(JB_SyxAcc, JB_Fn__StdNew(JB_Msg_Acc__, 1, JB_str_183, JB_str_184));
		JB_SetRef(JB_SyxAdj, JB_Fn__StdNew(JB_Msg_Adj__, 2, JB_str_185, JB_str_186));
		JB_SetRef(JB_SyxARel, JB_Fn__StdNew(JB_Msg_ARel__, 3, JB_str_187, JB_str_188));
		JB_SetRef(JB_SyxArr, JB_Fn__StdNew(JB_Msg_Arr__, 4, JB_str_189, JB_str_190));
		JB_SetRef(JB_SyxBack, JB_Fn__StdNew(JB_Msg_Back__, 5, JB_str_191, JB_str_192));
		JB_SetRef(JB_SyxSStr, JB_Fn__StdNew(JB_Msg_SStr__, 6, JB_str_193, JB_str_194));
		JB_SetRef(JB_SyxBra, JB_Fn__StdNew(JB_Msg_Bra__, 7, JB_str_195, JB_str_196));
		JB_SetRef(JB_SyxBRel, JB_Fn__StdNew(JB_Msg_BRel__, 8, JB_str_197, JB_str_198));
		JB_SetRef(JB_SyxChar, JB_Fn__StdNew(JB_Msg_Char__, 9, JB_str_199, JB_str_200));
		JB_SetRef(JB_SyxDecl, JB_Fn__StdNew(JB_Msg_Decl__, 10, JB_str_201, JB_str_202));
		JB_SetRef(JB_SyxDot, JB_Fn__StdNew(JB_Msg_Dot__, 11, JB_str_203, JB_str_204));
		JB_SetRef(JB_SyxEmb, JB_Fn__StdNew(JB_Msg_Emb__, 12, JB_str_205, JB_str_206));
		JB_SetRef(JB_SyxERel, JB_Fn__StdNew(JB_Msg_ERel__, 13, JB_str_207, JB_str_208));
		JB_SetRef(JB_SyxFunc, JB_Fn__StdNew(JB_Msg_Func__, 14, JB_str_209, JB_str_210));
		JB_SetRef(JB_SyxItem, JB_Fn__StdNew(JB_Msg_Item__, 15, JB_str_211, JB_str_0));
		JB_SetRef(JB_SyxList, JB_Fn__StdNew(JB_Msg_List__, 16, JB_str_147, JB_str_0));
		JB_SetRef(JB_SyxPrm, JB_Fn__StdNew(JB_Msg_Prm__, 17, JB_str_212, JB_str_213));
		JB_SetRef(JB_SyxName, JB_Fn__StdNew(JB_Msg_Name__, 18, JB_str_214, JB_str_0));
		JB_SetRef(JB_SyxNum, JB_Fn__StdNew(JB_Msg_Num__, 19, JB_str_215, JB_str_216));
		JB_SetRef(JB_SyxOat, JB_Fn__StdNew(JB_Msg_Oat__, 20, JB_str_217, JB_str_218));
		JB_SetRef(JB_SyxOpp, JB_Fn__StdNew(JB_Msg_Opp__, 21, JB_str_219, JB_str_220));
		JB_SetRef(JB_SyxSOpp, JB_Fn__StdNew(JB_Msg_SOpp__, 22, JB_str_221, JB_str_222));
		JB_SetRef(JB_SyxRel, JB_Fn__StdNew(JB_Msg_Rel__, 23, JB_str_223, JB_str_224));
		JB_SetRef(JB_SyxTRel, JB_Fn__StdNew(JB_Msg_TRel__, 24, JB_str_225, JB_str_226));
		JB_SetRef(JB_SyxSDot, JB_Fn__StdNew(JB_Msg_SDot__, 25, JB_str_227, JB_str_228));
		JB_SetRef(JB_SyxSCom, JB_Fn__StdNew(JB_Msg_SCom__, 26, JB_str_229, JB_str_230));
		JB_SetRef(JB_SyxSheb, JB_Fn__StdNew(JB_Msg_Sheb__, 27, JB_str_231, JB_str_232));
		JB_SetRef(JB_SyxStr, JB_Fn__StdNew(JB_Msg_Str__, 28, JB_str_233, JB_str_234));
		JB_SetRef(JB_SyxSThg, JB_Fn__StdNew(JB_Msg_SThg__, 29, JB_str_235, JB_str_236));
		JB_SetRef(JB_SyxThg, JB_Fn__StdNew(JB_Msg_Thg__, 30, JB_str_237, JB_str_238));
		JB_SetRef(JB_SyxChn, JB_Fn__StdNew(JB_Msg_Chn__, 31, JB_str_239, JB_str_240));
		JB_SetRef(JB_SyxTmp, JB_Fn__StdNew(JB_Msg_Tmp__, 32, JB_str_241, JB_str_242));
		JB_SetRef(JB_SyxAna, JB_Fn__StdNew(JB_Msg_Ana__, 33, JB_str_243, JB_str_244));
		JB_SetRef(JB_SyxSCnj, JB_Fn__StdNew(JB_Msg_SCnj__, 34, JB_str_245, JB_str_246));
		JB_SetRef(JB_SyxCnj, JB_Fn__StdNew(JB_Msg_Cnj__, 35, JB_str_247, JB_str_248));
		JB_SetRef(JB_SyxType, JB_Fn__StdNew(JB_Msg_Type__, 36, JB_str_249, JB_str_250));
		JB_SetRef(JB_SyxUnit, JB_Fn__StdNew(JB_Msg_Unit__, 37, JB_str_251, JB_str_0));
		JB_BaseMessagePosition = -1;
		JB_SetRef(JB_SyxBin, JB_Fn__StdNew(JB_Msg_Bin__, 38, JB_str_252, JB_str_253));
	};
	//// JB_API;
	JB_JB_API__Init_();
	//// Jeebox;
...
Message* JB_Str_ParseAs(JB_String* self, JB_String* name) {
	if ((!JB_Str_Exists(name)) or (JB_Str_SyntaxEquals(name, JB_str_146, true))) {
		...
		Message* _tmp320 = JB_Str_Parse(self);
		JB_Incr(_tmp320);
		Message* _tmp321 = JB_Msg_ParseReadable(_tmp320);
		JB_Incr(_tmp321);
		JB_Decr(_tmp320);
		JB_SafeDecr(_tmp321);
		return _tmp321;
...
JB_String* JB_Msg_Locate(Message* self) {
	...
	JB_Incr(rs);
	Message* _tmp319 = ((Message*)JB_Tree_Root(self));
	JB_Incr(_tmp319);
	JB_Object* r = _tmp319->Obj;
	JB_Incr(r);
	JB_Decr(_tmp319);
