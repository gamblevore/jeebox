...
int JB_Constants__Init_() {
	{
		...
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_238, JB_str_54));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_13, JB_str_86));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_239, JB_str_240));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_17, JB_str_241));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_53, JB_str_242));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_23, JB_str_243));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_56, JB_str_244));
		(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_245, JB_str_246));
		JB_SetRef(JB__Constants_JS_EscapeStr, JB_Dict__New());
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_247, JB_str_248));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_249, JB_str_250));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_13, JB_str_86));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_239, JB_str_240));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_17, JB_str_241));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_53, JB_str_242));
		(JB_Dict_ValueSet(JB__Constants_JS_EscapeStr, JB_str_56, JB_str_244));
		...
		JB_SetRef(JB__Constants_CSWordMiddle, JB_Str_UniCS(JB_str_251));
		JB_SetRef(JB__Constants_CSLettersOnly, JB_Str_UniCS(JB_str_252));
		JB_SetRef(JB__Constants_CSWordStart, JB_Str_UniCS(JB_str_253));
		JB_SetRef(JB__Constants_CSNum, JB__Constants_CSWordMiddle);
		JB_SetRef(JB__Constants_CSLine, JB_Str_Charset(JB_str_254, false));
		JB_SetRef(JB__Constants_CSAfterTemporal, JB_Str_Charset(JB_str_255, false));
		JB_SetRef(JB__Constants_CSLineBlack, JB_Str_Charset(JB_str_256, false));
...
void JB_Constants__InitConstants() {
	...
	(JB_Dict_ValueSet(JB__Constants_EscapeStr, JB_str_23, ((JB_String*)nil)));
...
JB_String* JB_Constants__Test() {
	...
	JB_String* natural_input = JB_Incr(JB_str_180);
	JB_String* list_input = JB_Incr(JB_str_181);
	if ((!(JB_TestCasting()))) {
		debugger;
	}
	JB_String* x = JB_Incr(JB_Str_UpperCase(JB_str_176));
	JB_Decr(x);
	LeakTester* LKT = JB_Incr(JB_Lk__New(JB_str_182));
	...
	if (JB_Str_Exists(list_input)) {
		...
		JB_Msg_Test(root, _tmP1, JB_str_176);
		JB_Decr(_tmP1);
		JB_String* _tmP2 = JB_Incr(JB_Msg_AST(root, nil));
		JB_Msg_Test(root, _tmP2, JB_str_177);
		JB_Decr(_tmP2);
		JB_Decr(root);
	} else {
		Message* root = JB_Incr(JB_Str_Parse(natural_input, nil));
		...
		JB_Msg_Test(root, R, JB_str_176);
		JB_Decr(root);
		JB_Str_Print(JB_str_183);
		JB_Str_Print(R);
		JB_Decr(R);
	}
	JB_Decr(list_input);
...
Message* JB_Tk__BarThings(int Start, Syntax Syx) {
	...
	JB__Tk_StopBars = 0;
	if ((!JB_Tk__ExpectEndChar(JB_str_119, true))) {
		JB_Decr(item);
		return nil;
	}
...
Message* JB_Tk__DotSub(Syntax fn, int Start) {
	...
	if (JB_Tk__EatString(JB_str_117)) {
	...
	if (RealBits == kJB__Tk_EndOfLine) {
		...
		if (((bool)(RealBits & kJB__Tk_IllegalChar))) {
			...
			if (JB_byte_CanPrintAsNormalChar(illegal)) {
				...
				JB_FS_AppendString(Err, JB_str_34);
...
bool JB_Tk__ExpectEndChar(JB_String* s, bool Expect) {
	...
	if (Expect) {
		FastString* _fs_fs = JB_Incr(JB_FS__New());
		JB_FS_AppendString(_fs_fs, JB_str_135);
...
Message* JB_Tk__fAccess(int Start) {
	...
	JB_Tk__ParseLoop(prms, JB_str_136);
	JB_Decr(prms);
	JB_SafeDecr(result);
	return result;
}
...
Message* JB_Tk__fArgOpen(int Start) {
	...
	JB_Tk__ParseLoop(Result, JB_str_144);
	JB_SafeDecr(Result);
	return Result;
}
...
Message* JB_Tk__fBackTick(int Start) {
	return JB_Tk__fStrSub(Start, JB_str_132, JB_SyxBack);
}
...
Message* JB_Tk__fBracket(int Start) {
	...
	bool Lines = JB_Tk__ParseLoop(Result, JB_str_140);
...
Message* JB_Tk__fComment(int Start) {
	...
	while (true) {
		...
		if ((!JB_int_Found(i))) {
			JB_Decr(d);
			return JB_Tk__ErrorAdd(JB_str_142, Start);
...
Message* JB_Tk__fIndentError(int Start) {
	JB_FreeIfDead(JB_Tk__ErrorAdd(JB_str_155, Start));
	return nil;
}
...
Message* JB_Tk__fMsgList(int Start) {
	...
	if (bad) {
		JB_Decr(name);
		Message* _tmP = JB_Incr(JB_Tk__ErrorAdd(JB_str_137, bad->Position));
		JB_Decr(bad);
		JB_SafeDecr(_tmP);
		return _tmP;
	}
	...
	while (JB_Tk__ConsumeLines(JB_SyxList)) {
		...
		if (JB_Str_OperatorEndsWith(item->Name, JB_str_76)) {
			JB_Decr(Result);
			Message* _tmP1 = JB_Incr(JB_Tk__ErrorAdd(JB_str_138, item->Position));
			JB_Decr(item);
			JB_SafeDecr(_tmP1);
			return _tmP1;
		}
...
Message* JB_Tk__fOppBracket(int Start) {
	...
	JB_Decr(opp2);
	JB_Decr(opp1);
	JB_Tk__ExpectEndChar(JB_str_140, true);
...
Message* JB_Tk__fString(int Start) {
	...
	while (true) {
		...
		if (C0 == '{') {
			StrPos = JB_Tk__EmbeddedCode(JB_str_144, result, JB_SyxList);
		} else {
		...
		if (StrPos > After) {
			JB_Decr(result);
			return JB_Tk__ErrorAdd(JB_str_146, NewStrPos);
		}
	};
	(JB_Tk__NextStartSet(After + 1));
...
void JB_Tk__Init() {
	...
	JB_Decr(_tmP2);
	JB_SetRef(JB__Tk_EndOfLineMarker, JB_Msg__NewWithSyntaxStr(nil, JB_str_18));
	Array* numb = JB_Incr(JB_Str_ByteSplit(JB_str_112));
	JB_String* op_comp = JB_Incr(JB_str_113);
	JB_String* op_math = JB_Incr(JB_str_114);
	JB_String* op_math2 = JB_Incr(JB_str_115);
	JB_Decr(op_math2);
	JB_String* PFix = JB_Incr(JB_str_116);
	TokHan* H = JB_Incr(({
	...
	(JB_Tk__TokenSet(JB_str_117, _tmP4));
	JB_Decr(_tmP4);
	...
	(JB_Tk__TokenSet(JB_str_118, _tmP5));
	JB_Decr(_tmP5);
	...
	(JB_Tk__TokenSet(JB_str_28, _tmP7));
	JB_Decr(_tmP7);
	...
	(JB_Tk__TokenSet(JB_str_119, _tmP8));
	JB_Decr(_tmP8);
	...
	(JB_Tk__TokenSet(JB_str_120, _tmP9));
	JB_Decr(_tmP9);
	...
	(JB_Tk__TokenSet(JB_str_121, _tmP10));
	JB_Decr(_tmP10);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_122, kJB__Tk_ThingSyx, JB_Tk__fBeforeRelMinus);
	...
	JB_Decr(PFix);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_123, kJB__Tk_ThingSyx, JB_Tk__fBeforeRel);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_123, kJB__Tk_FuncAfterNormal, JB_Tk__fAfterRel);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_124, kJB__Tk_ThingSyx, JB_Tk__fArgOpen);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_125, kJB__Tk_TmpOpp, JB_Tk__fTemporalRel);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_126, kJB__Tk_TmpOpp, JB_Tk__fTemporalRelSyx);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_42, kJB__Tk_ThingSyx, JB_Tk__fOpAsThing);
	...
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_127, kJB__Tk_EndOfLine, JB_Tk__fCommentLine);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_128, kJB__Tk_EndOfLine, JB_Tk__fComment);
	...
	JB_Decr(op_math);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_129, kJB__Tk_OppYoda, JB_Tk__fOppYoda);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_130, kJB__Tk_ThingSyx, JB_Tk__fBunchOfThings);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_131, kJB__Tk_ThingSyx, JB_Tk__fThingSyx);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_53, kJB__Tk_ThingSyx, JB_Tk__fString);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_23, kJB__Tk_ThingSyx, JB_Tk__fChr);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_132, kJB__Tk_ThingSyx, JB_Tk__fBackTick);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_65, kJB__Tk_ThingSyx, JB_Tk__fSuperStr);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_133, kJB__Tk_ThingSyx, JB_Tk__fBeforeRel);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_99, kJB__Tk_ThingSyx, JB_Tk__fBinary);
	JB_Tk__TokenzWithStrIntParsehandler(JB_str_134, kJB__Tk_EndContainer, JB_Tk__fEndOfLine);
...
Message* JB_Tk__InvisArgEscapeNested(Message* curr) {
	while (true) {
		...
		if ((!JB_Msg_SyntaxEqualsWithSyntaxBool(p, JB_SyxTmp, false))) {
			return curr;
		}
		JB_DoAt(1);
		curr = p;
...
Message* JB_Tk__MakeRel(Message* first, int Bits) {
	...
	JB_Tree_SyntaxAppend(Rel, first);
	JB_Tree_SyntaxAppend(Rel, opp);
	if (JB_Tk__EatString(JB_str_151)) {
		Rel->Func = JB_SyxERel;
	}
...
int JB_Tk__MessageErrorSub(FastString* fs, int num, int ButFound) {
	if ((!JB__Tk_ErrorNames)) {
		...
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Temporal, JB_str_160));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_TemporalSyx, JB_str_161));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_TemporalWord, JB_str_162));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_EndOfLine, JB_str_157));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_colon, JB_str_163));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppSyx, JB_str_164));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppWord, JB_str_164));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Opp, JB_str_164));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_OppTemporal, JB_str_165));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_FuncAfterBar, JB_str_166));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_Thing, JB_str_167));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_ThingWord, JB_str_167));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_ThingSyx, JB_str_168));
		(JB_Dict_SyntaxAccessSet(JB__Tk_ErrorNames, kJB__Tk_IllegalChar, JB_str_169));
	}
	if (JB_FS_Length(fs)) {
		JB_FS_AppendString(fs, JB_str_170);
	} else {
...
Message* JB_Tk__ReRoute(Message* cnj, Message* output) {
	...
	if ((!(f))) {
		JB_Decr(f);
		return JB_Tk__ErrorAdd(JB_str_152, JB_Tk__NextStart());
	}
...
bool JB_Platform__linux() {
	return (({
		...
		bool _tmP227 = JB_Str_SyntaxEquals(_tmP228, JB_str_37, false);
		JB_Decr(_tmP228);
		 _tmP227;
	}));
}
...
JB_String* JB_EntityTest() {
	...
	(JB_Dict_ValueSet(repl, JB_str_86, JB_str_13));
...
int JB_Init_() {
	{
		JB_SetRef(JB_str_335, JB_Str3("Binary", 6));
		JB_SetRef(JB_str_334, JB_Str3("bin", 3));
		JB_SetRef(JB_str_333, JB_Str3("To_Do_List", 10));
		JB_SetRef(JB_str_332, JB_Str3("todo", 4));
		JB_SetRef(JB_str_331, JB_Str3("Bunch_Of_Things", 15));
		JB_SetRef(JB_str_330, JB_Str3("bnch", 4));
		JB_SetRef(JB_str_329, JB_Str3("Message_List", 12));
		JB_SetRef(JB_str_328, JB_Str3("msg", 3));
		JB_SetRef(JB_str_327, JB_Str3("Question", 8));
		JB_SetRef(JB_str_326, JB_Str3("ask", 3));
		JB_SetRef(JB_str_325, JB_Str3("yopp", 4));
		JB_SetRef(JB_str_324, JB_Str3("Yoda_Relationship", 17));
		JB_SetRef(JB_str_323, JB_Str3("yoda", 4));
		JB_SetRef(JB_str_322, JB_Str3("Typecast", 8));
		JB_SetRef(JB_str_321, JB_Str3("type", 4));
		JB_SetRef(JB_str_320, JB_Str3("Conjugate", 9));
		JB_SetRef(JB_str_319, JB_Str3("cnj", 3));
		JB_SetRef(JB_str_318, JB_Str3("Syntactic_Conjugate", 19));
		JB_SetRef(JB_str_317, JB_Str3("scnj", 4));
		JB_SetRef(JB_str_316, JB_Str3("Temporal_Reference", 18));
		JB_SetRef(JB_str_315, JB_Str3("ana", 3));
		JB_SetRef(JB_str_314, JB_Str3("Statement", 9));
		JB_SetRef(JB_str_313, JB_Str3("tmp", 3));
		JB_SetRef(JB_str_312, JB_Str3("Thing", 5));
		JB_SetRef(JB_str_311, JB_Str3("thg", 3));
		JB_SetRef(JB_str_310, JB_Str3("Syntactic_Thing", 15));
		JB_SetRef(JB_str_309, JB_Str3("sthg", 4));
		JB_SetRef(JB_str_308, JB_Str3("String", 6));
		JB_SetRef(JB_str_307, JB_Str3("str", 3));
		JB_SetRef(JB_str_306, JB_Str3("Shebang", 7));
		JB_SetRef(JB_str_305, JB_Str3("sheb", 4));
		JB_SetRef(JB_str_304, JB_Str3("Syntactic_Comment", 17));
		JB_SetRef(JB_str_303, JB_Str3("scom", 4));
		JB_SetRef(JB_str_302, JB_Str3("Self_Dot", 8));
		JB_SetRef(JB_str_301, JB_Str3("sdot", 4));
		JB_SetRef(JB_str_300, JB_Str3("Temporal_Relationship", 21));
		JB_SetRef(JB_str_299, JB_Str3("trel", 4));
		JB_SetRef(JB_str_298, JB_Str3("Relationship", 12));
		JB_SetRef(JB_str_297, JB_Str3("rel", 3));
		JB_SetRef(JB_str_296, JB_Str3("Syntactic_Operator", 18));
		JB_SetRef(JB_str_295, JB_Str3("sopp", 4));
		JB_SetRef(JB_str_294, JB_Str3("Operator", 8));
		JB_SetRef(JB_str_293, JB_Str3("opp", 3));
		JB_SetRef(JB_str_292, JB_Str3("Operator_As_Thing", 17));
		JB_SetRef(JB_str_291, JB_Str3("oat", 3));
		JB_SetRef(JB_str_290, JB_Str3("unit", 4));
		JB_SetRef(JB_str_289, JB_Str3("Number", 6));
		JB_SetRef(JB_str_288, JB_Str3("num", 3));
		JB_SetRef(JB_str_287, JB_Str3("name", 4));
		JB_SetRef(JB_str_286, JB_Str3("item", 4));
		JB_SetRef(JB_str_285, JB_Str3("Function", 8));
		JB_SetRef(JB_str_284, JB_Str3("func", 4));
		JB_SetRef(JB_str_283, JB_Str3("Equals_Relationship", 19));
		JB_SetRef(JB_str_282, JB_Str3("erel", 4));
		JB_SetRef(JB_str_281, JB_Str3("Embedded_Value", 14));
		JB_SetRef(JB_str_280, JB_Str3("emb", 3));
		JB_SetRef(JB_str_279, JB_Str3("Dot", 3));
		JB_SetRef(JB_str_278, JB_Str3("dot", 3));
		JB_SetRef(JB_str_277, JB_Str3("Declaration", 11));
		JB_SetRef(JB_str_276, JB_Str3("decl", 4));
		JB_SetRef(JB_str_275, JB_Str3("Character", 9));
		JB_SetRef(JB_str_274, JB_Str3("Before_Relationship", 19));
		JB_SetRef(JB_str_273, JB_Str3("brel", 4));
		JB_SetRef(JB_str_272, JB_Str3("Bracket", 7));
		JB_SetRef(JB_str_271, JB_Str3("bra", 3));
		JB_SetRef(JB_str_270, JB_Str3("Super_String", 12));
		JB_SetRef(JB_str_269, JB_Str3("sstr", 4));
		JB_SetRef(JB_str_268, JB_Str3("Backtick", 8));
		JB_SetRef(JB_str_267, JB_Str3("back", 4));
		JB_SetRef(JB_str_266, JB_Str3("Array", 5));
		JB_SetRef(JB_str_265, JB_Str3("arr", 3));
		JB_SetRef(JB_str_264, JB_Str3("After_Relationship", 18));
		JB_SetRef(JB_str_263, JB_Str3("arel", 4));
		JB_SetRef(JB_str_262, JB_Str3("Adjective", 9));
		JB_SetRef(JB_str_261, JB_Str3("adj", 3));
		JB_SetRef(JB_str_260, JB_Str3("Access", 6));
		JB_SetRef(JB_str_259, JB_Str3("acc", 3));
		JB_SetRef(JB_str_258, JB_Str3("Argument", 8));
		JB_SetRef(JB_str_257, JB_Str3("arg", 3));
		JB_SetRef(JB_str_256, JB_Str3("\n\r,", 3));
		JB_SetRef(JB_str_255, JB_Str3(" ,:\t\n\r\\", 7));
		JB_SetRef(JB_str_254, JB_Str3("\n\r", 2));
		JB_SetRef(JB_str_253, JB_Str3("AZaz__", 6));
		JB_SetRef(JB_str_252, JB_Str3("AZaz", 4));
		JB_SetRef(JB_str_251, JB_Str3("AZaz09__", 8));
		JB_SetRef(JB_str_250, JB_Str3("\\f", 2));
		JB_SetRef(JB_str_249, JB_Str3("", 1));
		JB_SetRef(JB_str_248, JB_Str3("\\b", 2));
		JB_SetRef(JB_str_247, JB_Str3("", 1));
		JB_SetRef(JB_str_246, JB_Str3("\\&#", 3));
		...
		JB_SetRef(JB_str_240, JB_Str3("\\r", 2));
		JB_SetRef(JB_str_239, JB_Str3("\r", 1));
		JB_SetRef(JB_str_238, JB_Str3("\0", 1));
		JB_SetRef(JB_str_237, JB_Str3("[0;1;33m", 9));
		JB_SetRef(JB_str_236, JB_Str3("[0;1;32m", 9));
		JB_SetRef(JB_str_235, JB_Str3("[0;1;31m", 9));
		JB_SetRef(JB_str_234, JB_Str3("[0;1m", 6));
		JB_SetRef(JB_str_233, JB_Str3("[0m", 4));
		JB_SetRef(JB_str_232, JB_Str3("@JBSave\n", 8));
		JB_SetRef(JB_str_231, JB_Str3("unusedtype", 10));
		JB_SetRef(JB_str_230, JB_Str3("point4", 6));
		JB_SetRef(JB_str_229, JB_Str3("point3", 6));
		JB_SetRef(JB_str_228, JB_Str3("point2", 6));
		JB_SetRef(JB_str_227, JB_Str3("ipoint4", 7));
		JB_SetRef(JB_str_226, JB_Str3("ipoint3", 7));
		JB_SetRef(JB_str_225, JB_Str3("ipoint2", 7));
		JB_SetRef(JB_str_224, JB_Str3("s16", 3));
		JB_SetRef(JB_str_223, JB_Str3("u16", 3));
		JB_SetRef(JB_str_222, JB_Str3("s8", 2));
		JB_SetRef(JB_str_221, JB_Str3("f16", 3));
		JB_SetRef(JB_str_220, JB_Str3("f64", 3));
		JB_SetRef(JB_str_219, JB_Str3("u64", 3));
		JB_SetRef(JB_str_218, JB_Str3("s64", 3));
		JB_SetRef(JB_str_217, JB_Str3("u32", 3));
		JB_SetRef(JB_str_216, JB_Str3("uint", 4));
		JB_SetRef(JB_str_215, JB_Str3("int", 3));
		JB_SetRef(JB_str_214, JB_Str3("mat4", 4));
		JB_SetRef(JB_str_213, JB_Str3("mat2", 4));
		JB_SetRef(JB_str_212, JB_Str3("vec4", 4));
		JB_SetRef(JB_str_211, JB_Str3("vec3", 4));
		JB_SetRef(JB_str_210, JB_Str3("vec2", 4));
		JB_SetRef(JB_str_209, JB_Str3("double", 6));
		JB_SetRef(JB_str_208, JB_Str3("float", 5));
		JB_SetRef(JB_str_207, JB_Str3("hfloat", 6));
		JB_SetRef(JB_str_206, JB_Str3("ivec4", 5));
		JB_SetRef(JB_str_205, JB_Str3("ivec3", 5));
		JB_SetRef(JB_str_204, JB_Str3("ivec2", 5));
		JB_SetRef(JB_str_203, JB_Str3("uint64", 6));
		JB_SetRef(JB_str_202, JB_Str3("int64", 5));
		JB_SetRef(JB_str_201, JB_Str3("s16x4", 5));
		JB_SetRef(JB_str_200, JB_Str3("s16x3", 5));
		JB_SetRef(JB_str_199, JB_Str3("s16x2", 5));
		JB_SetRef(JB_str_198, JB_Str3("int16", 5));
		JB_SetRef(JB_str_197, JB_Str3("uint16", 6));
		JB_SetRef(JB_str_196, JB_Str3("sbyte4", 6));
		JB_SetRef(JB_str_195, JB_Str3("sbyte3", 6));
		JB_SetRef(JB_str_194, JB_Str3("sbyte2", 6));
		JB_SetRef(JB_str_193, JB_Str3("sbyte", 5));
		JB_SetRef(JB_str_192, JB_Str3("byte4", 5));
		JB_SetRef(JB_str_191, JB_Str3("byte3", 5));
		JB_SetRef(JB_str_190, JB_Str3("byte2", 5));
		JB_SetRef(JB_str_189, JB_Str3("char", 4));
		JB_SetRef(JB_str_188, JB_Str3("u8", 2));
		JB_SetRef(JB_str_187, JB_Str3("').", 3));
		JB_SetRef(JB_str_186, JB_Str3("' (found '", 10));
		JB_SetRef(JB_str_185, JB_Str3("Can’t parse number '", 22));
		JB_SetRef(JB_str_184, JB_Str3("Not indented properly\?", 22));
		JB_SetRef(JB_str_183, JB_Str3("Manual inspection:\n\n", 20));
		JB_SetRef(JB_str_182, JB_Str3("entities", 8));
		JB_SetRef(JB_str_181, JB_Str3("«tmp \"how\"\n\t«oat \n\t\t«opp \"can\"»\n\t\t«trel \n\t\t\t«rel \n\t\t\t\t«thg \"someone\"»\n\t\t\t\t«opp \"fall\"»\n\t\t\t\t«adj \n\t\t\t\t\t«thg \"so\"»\n\t\t\t\t\t«thg \"far\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«cnj \"without\"»\n\t\t\t«oat \n\t\t\t\t«opp \"realising\"»\n\t\t\t\t«ana \"it\"»\n\t\t\t»\n\t\t»\n\t»\n»\n«tmp \"story\"\n\t«thg \"X\"»\n\t«list \n\t\t«trel \n\t\t\t«rel \n\t\t\t\t«thg \"x\"»\n\t\t\t\t«bra \n\t\t\t\t\t«adj \n\t\t\t\t\t\t«opp \"a\"»\n\t\t\t\t\t\t«opp \"b\"»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t\t«thg \"c\"»\n\t\t\t»\n\t\t\t«scnj \"-->\"»\n\t\t\t«rel \n\t\t\t\t«name \"sonic\"\n\t\t\t\t\t«adj \n\t\t\t\t\t\t«thg \"the\"»\n\t\t\t\t\t\t«thg \"hedgedog\"»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t\t«opp \"or\"»\n\t\t\t\t«name \"jeebox\"»\n\t\t\t»\n\t\t»\n\t\t«trel \n\t\t\t«rel \n\t\t\t\t«thg \"A\"»\n\t\t\t\t«bra \n\t\t\t\t\t«rel \n\t\t\t\t\t\t«adj \n\t\t\t\t\t\t\t«bra \n\t\t\t\t\t\t\t\t«adj \n\t\t\t\t\t\t\t\t\t«opp \"B\"»\n\t\t\t\t\t\t\t\t\t«opp \"C\"»\n\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t«opp \"D\"»\n\t\t\t\t\t\t»\n\t\t\t\t\t\t«opp \"E\"»\n\t\t\t\t\t\t«adj \n\t\t\t\t\t\t\t«opp \"F\"»\n\t\t\t\t\t\t\t«opp \"G\"»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t\t«thg \"I\"»\n\t\t\t»\n\t\t\t«cnj \"W\"»\n\t\t\t«rel \n\t\t\t\t«thg \"A\"»\n\t\t\t\t«opp \"t\"»\n\t\t\t\t«list \n\t\t\t\t\t«item \n\t\t\t\t\t\t«thg \"o\"»\n\t\t\t\t\t\t«thg \"c\"»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t»\n»\n«tmp \"za\"\n\t«thg \"a\"»\n\t«arg \n\t\t«decl \n\t\t\t«num \"1\"»\n\t\t\t«rel \n\t\t\t\t«thg \"x\"»\n\t\t\t\t«opp \"*\"»\n\t\t\t\t«rel \n\t\t\t\t\t«thg \"y\"»\n\t\t\t\t\t«opp \"*\"»\n\t\t\t\t\t«rel \n\t\t\t\t\t\t«thg \"z\"»\n\t\t\t\t\t\t«opp \"=\"»\n\t\t\t\t\t\t«brel \"\?\"\n\t\t\t\t\t\t\t«arel \"\?\"\n\t\t\t\t\t\t\t\t«list »\n\t\t\t\t\t\t\t»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t\t«decl \n\t\t\t«emb »\n\t\t\t«rel \n\t\t\t\t«thg \"a\"»\n\t\t\t\t«opp \"=\"»\n\t\t\t\t«rel \n\t\t\t\t\t«thg \"c\"»\n\t\t\t\t\t«opp \"or\"»\n\t\t\t\t\t«thg \"d\"»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t\t«list \n\t\t\t«thg \"a\"»\n\t\t»\n\t\t«list \n\t\t\t«trel \n\t\t\t\t«arr »\n\t\t\t\t«scnj \"-->\"»\n\t\t\t\t«rel \n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t«opp \"+\"»\n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«thg \"b\"»\n\t\t\t«item \n\t\t\t\t«thg \"d\"»\n\t\t\t\t«thg \"e\"»\n\t\t\t»\n\t\t»\n\t\t«arr \n\t\t\t«type \n\t\t\t\t«thg \"d\"»\n\t\t\t\t«num \"0\"»\n\t\t\t»\n\t\t\t«item \n\t\t\t\t«rel \n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t«opp \"+\"»\n\t\t\t\t\t«thg \"b\"»\n\t\t\t\t»\n\t\t\t\t«thg \"c\"»\n\t\t\t»\n\t\t\t«arr \n\t\t\t\t«thg \"c\"»\n\t\t\t\t«thg \"d\"»\n\t\t\t»\n\t\t»\n\t»\n»\n«bin \"binary encoding oof\"»\n«tmp \"function\"\n\t«thg \"name\"»\n\t«list \n\t\t«decl \n\t\t\t«item \n\t\t\t\t«thg \"x\"»\n\t\t\t\t«rel \n\t\t\t\t\t«thg \"y\"»\n\t\t\t\t\t«opp \"+\"»\n\t\t\t\t\t«thg \"z\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«adj \n\t\t\t\t«thg \"a\"»\n\t\t\t\t«thg \"b\"»\n\t\t\t»\n\t\t»\n\t\t«decl \n\t\t\t«emb »\n\t\t»\n\t»\n\t«arg \n\t\t«item \n\t\t\t«thg \"label\"»\n\t\t\t«func \n\t\t\t\t«thg \"func\"»\n\t\t\t\t«list »\n\t\t\t»\n\t\t»\n\t\t«str \"'\\\"striñg test\\\"'\"»\n\t\t«back \"\\\\n\\\\(\\&#x134;\\&#xEB;\\\\C3\\\\AA\\\\C6\\\\81o\\&#9447;\\\\)\"»\n\t\t«erel \n\t\t\t«brel \"--\"\n\t\t\t\t«arel \"--\"\n\t\t\t\t\t«thg \"x\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«opp \"mod\"»\n\t\t\t«char \"ioi '\\\"\\\"'oio\"»\n\t\t»\n\t\t«dot \"y\"\n\t\t\t«dot \"size\"\n\t\t\t\t«thg \"y\"»\n\t\t\t\t«list \n\t\t\t\t\t«arel \"\?\"\n\t\t\t\t\t\t«sthg \"...\"»\n\t\t\t\t\t»\n\t\t\t\t\t«sthg \"...\"»\n\t\t\t\t\t«sstr \"s`t`r\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«list \n\t\t\t\t«brel \"-\"\n\t\t\t\t\t«thg \"z\"»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t\t«rel \n\t\t\t«thg \"x\"»\n\t\t\t«opp \"=\"»\n\t\t\t«rel \n\t\t\t\t«str \n\t\t\t\t\t«emb \"{ab\"»\n\t\t\t\t\t«thg \"x\"»\n\t\t\t\t\t«emb \" \"»\n\t\t\t\t\t«thg \"y\"»\n\t\t\t\t\t«thg \"z\"»\n\t\t\t\t\t«emb \"ab}\"»\n\t\t\t\t»\n\t\t\t\t«opp \"+\"»\n\t\t\t\t«back \"abc\"»\n\t\t\t»\n\t\t»\n\t\t«erel \n\t\t\t«type \n\t\t\t\t«thg \"z\"»\n\t\t\t\t«list \n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«opp \"+\"»\n\t\t\t«sdot \"exit\"\n\t\t\t\t«list \n\t\t\t\t\t«item \n\t\t\t\t\t\t«thg \"code\"»\n\t\t\t\t\t\t«num \"-3\"»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t\t«tmp \"for\"\n\t\t\t«bra \n\t\t\t\t«rel \n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t«opp \"in\"»\n\t\t\t\t\t«thg \"b\"»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«arg \n\t\t\t\t«rel \n\t\t\t\t\t«brel \"$\"\n\t\t\t\t\t\t«arel \"$\"\n\t\t\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t\t«opp \"=\"»\n\t\t\t\t\t«rel \n\t\t\t\t\t\t«brel \"&\"\n\t\t\t\t\t\t\t«thg \"b\"»\n\t\t\t\t\t\t»\n\t\t\t\t\t\t«opp \"+\"»\n\t\t\t\t\t\t«brel \"!\"\n\t\t\t\t\t\t\t«arel \"!\"\n\t\t\t\t\t\t\t\t«thg \"c\"»\n\t\t\t\t\t\t\t»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t\t«tmp \"if\"\n\t\t\t«rel \n\t\t\t\t«bra \n\t\t\t\t\t«rel \n\t\t\t\t\t\t«char \"a\"»\n\t\t\t\t\t\t«opp \"&&&\"»\n\t\t\t\t\t\t«rel \n\t\t\t\t\t\t\t«thg \"b\"»\n\t\t\t\t\t\t\t«opp \"===\"»\n\t\t\t\t\t\t\t«rel \n\t\t\t\t\t\t\t\t«brel \"!\"\n\t\t\t\t\t\t\t\t\t«brel \"!\"\n\t\t\t\t\t\t\t\t\t\t«brel \"!\"\n\t\t\t\t\t\t\t\t\t\t\t«thg \"b\"»\n\t\t\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t\t«opp \"|||\"»\n\t\t\t\t\t\t\t\t«thg \"c\"»\n\t\t\t\t\t\t\t»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t\t«opp \"^^^\"»\n\t\t\t\t«bra \n\t\t\t\t\t«rel \n\t\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t\t«opp \"~>=\"»\n\t\t\t\t\t\t«thg \"b\"»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t\t«arg \n\t\t\t\t«tmp \"if\"\n\t\t\t\t\t«thg \"a\"»\n\t\t\t\t\t«arg \":\"\n\t\t\t\t\t\t«tmp \"return\"\n\t\t\t\t\t\t\t«rel \n\t\t\t\t\t\t\t\t«unit \"cm\"\n\t\t\t\t\t\t\t\t\t«num \"0xff_\"»\n\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t\t«opp \"+\"»\n\t\t\t\t\t\t\t\t«unit \"miles\"\n\t\t\t\t\t\t\t\t\t«num \"10_\"»\n\t\t\t\t\t\t\t\t»\n\t\t\t\t\t\t\t»\n\t\t\t\t\t\t»\n\t\t\t\t\t»\n\t\t\t\t»\n\t\t\t»\n\t\t»\n\t»\n»\n", 4262));
		JB_SetRef(JB_str_180, JB_Str3("// Jeebox self-test file.\n// Tests all syntax in small amount of code.\n// Does not make sense in any other way.\n\nhow %can someone fall so~far #without %realising –it\n\n\nstory X (\n\tx (a ~ b) c\n\t\t\t-->\n\tsonic@the~hedgedog or @jeebox\n\n\tA ((B~C)~D E F~G) I #W A t (o:c)\n)\n\n\n\n\nza a {\n\t|1| x * y * z = \?()\?\n\t|| a = c or d\n\t(a,)\n\t([] --> a + a,b,d:e)\n\t[d|0|, a+b:c, [c, d]] /* /*\n\t\tnested comments!\n\t\t*/ I found it useful.*/\n\n\t// }\n}\n\n#(OcbkON9v86LkOszaQMvd86zlPW)#\n\nfunction name (|x:y+z| a ~ b, ||) {\n\tlabel: func()\n\t\"'\\\"stri\\c3\\b1g test\\\"'\"\n\t`\\n\\(&#x134;&#xEB;\\C3\\AA\\C6\\81o&#9447;\\)`\n\t--x-- mod= 'ioi \\'\"\"\\'oio'\n\t\n\ty.size(...\?, ..., <(`s`t`r`)>).y(-z)\n\n\tx = \"{ab$\{x} $\{y}$\{z}ab}\" + `abc`\n\tz|(a,)| += .exit(code: -3)\n\n\tfor (a in b) {\n\t\t\$a$ = &b + !c!\n\t}\n\n\tif ('a' &&& b === !!!b ||| c) ^^^ (a ~>= b) {\n\t\tif a: return 0xff_cm + 10_miles\n\t}\n}\n\n\n", 838));
		JB_SetRef(JB_str_179, JB_Str3(": Failed\n\n", 10));
		JB_SetRef(JB_str_178, JB_Str3("Test ", 5));
		JB_SetRef(JB_str_177, JB_Str3("list", 4));
		JB_SetRef(JB_str_176, JB_Str3("normal", 6));
		JB_SetRef(JB_str_175, JB_Str3(" in ", 4));
		JB_SetRef(JB_str_174, JB_Str3("' at byte: ", 11));
		JB_SetRef(JB_str_173, JB_Str3(": '", 3));
		JB_SetRef(JB_str_172, JB_Str3("(nil)", 5));
		JB_SetRef(JB_str_171, JB_Str3("something else", 14));
		JB_SetRef(JB_str_170, JB_Str3(" / ", 3));
		JB_SetRef(JB_str_169, JB_Str3("illegal-character", 17));
		JB_SetRef(JB_str_168, JB_Str3("something", 9));
		JB_SetRef(JB_str_167, JB_Str3("a name", 6));
		JB_SetRef(JB_str_166, JB_Str3("'|'", 3));
		JB_SetRef(JB_str_165, JB_Str3("temporal-operator", 17));
		JB_SetRef(JB_str_164, JB_Str3("operator", 8));
		JB_SetRef(JB_str_163, JB_Str3("':'", 3));
		JB_SetRef(JB_str_162, JB_Str3("statement", 9));
		JB_SetRef(JB_str_161, JB_Str3("declaration", 11));
		JB_SetRef(JB_str_160, JB_Str3("temporal", 8));
		JB_SetRef(JB_str_159, JB_Str3("Found illegal-character ", 24));
		JB_SetRef(JB_str_158, JB_Str3("end-of-data", 11));
		JB_SetRef(JB_str_157, JB_Str3("end-of-line", 11));
		JB_SetRef(JB_str_156, JB_Str3("Unexpected ", 11));
		JB_SetRef(JB_str_155, JB_Str3("No tabs after a space!", 22));
		JB_SetRef(JB_str_154, JB_Str3("Indents mismatch", 16));
		JB_SetRef(JB_str_153, JB_Str3("Expected a letter (aA-zZ, '_', etc).", 36));
		JB_SetRef(JB_str_152, JB_Str3("Can’t use temporals like this.", 32));
		JB_SetRef(JB_str_151, JB_Str3("=", 1));
		JB_SetRef(JB_str_150, JB_Str3("Must be a proper name (a-z or letters)", 38));
		JB_SetRef(JB_str_149, JB_Str3("' not allowed.", 14));
		JB_SetRef(JB_str_148, JB_Str3("Nested '", 8));
		JB_SetRef(JB_str_147, JB_Str3("#/", 2));
		JB_SetRef(JB_str_146, JB_Str3("Embedded expression didn't close.", 33));
		JB_SetRef(JB_str_145, JB_Str3("String didn't close!", 20));
		JB_SetRef(JB_str_144, JB_Str3("}", 1));
		JB_SetRef(JB_str_143, JB_Str3("Can’t have '=' after this operator.", 37));
		JB_SetRef(JB_str_142, JB_Str3("Comment did not close.", 22));
		JB_SetRef(JB_str_141, JB_Str3("Char didn't close!", 18));
		JB_SetRef(JB_str_140, JB_Str3(")", 1));
		JB_SetRef(JB_str_139, JB_Str3("String did not close.", 21));
		JB_SetRef(JB_str_138, JB_Str3("Needs a space here.", 19));
		JB_SetRef(JB_str_137, JB_Str3("Unexpected syntax", 17));
		JB_SetRef(JB_str_136, JB_Str3("]", 1));
		JB_SetRef(JB_str_135, JB_Str3("Missing '", 9));
		JB_SetRef(JB_str_134, JB_Str3("] ) } #/ »", 11));
		JB_SetRef(JB_str_133, JB_Str3("~~~", 3));
		JB_SetRef(JB_str_132, JB_Str3("`", 1));
		JB_SetRef(JB_str_131, JB_Str3("...", 3));
		JB_SetRef(JB_str_130, JB_Str3("‡", 3));
		JB_SetRef(JB_str_129, JB_Str3("¬", 2));
		JB_SetRef(JB_str_128, JB_Str3("/*", 2));
		JB_SetRef(JB_str_127, JB_Str3("//", 2));
		JB_SetRef(JB_str_126, JB_Str3("--> <-- <-->", 12));
		JB_SetRef(JB_str_125, JB_Str3("#", 1));
		JB_SetRef(JB_str_124, JB_Str3("{", 1));
		JB_SetRef(JB_str_123, JB_Str3("&", 1));
		JB_SetRef(JB_str_122, JB_Str3("- *", 3));
		...
		JB_SetRef(JB_str_116, JB_Str3("++ -- $ ! \? ...", 15));
		...
		JB_SetRef(JB_str_46, JB_Str3("1", 1));
		JB_SetRef(JB_str_45, JB_Str3("(\n", 2));
		JB_SetRef(JB_str_44, JB_Str3("nil,", 4));
		JB_SetRef(JB_str_43, JB_Str3("This class has no saving function for it.", 41));
		JB_SetRef(JB_str_42, JB_Str3("%", 1));
		JB_SetRef(JB_str_41, JB_Str3("  deviance: ", 12));
		JB_SetRef(JB_str_40, JB_Str3("  found: ", 9));
		JB_SetRef(JB_str_39, JB_Str3("expected: ", 10));
		JB_SetRef(JB_str_38, JB_Str3("win", 3));
		JB_SetRef(JB_str_37, JB_Str3("lin", 3));
		JB_SetRef(JB_str_36, JB_Str3("osx", 3));
		JB_SetRef(JB_str_35, JB_Str3("Expected class ", 15));
		JB_SetRef(JB_str_34, JB_Str3("0x", 2));
		JB_SetRef(JB_str_33, JB_Str3("This class has no loader for it.", 32));
		JB_SetRef(JB_str_32, JB_Str3("Bad data found.", 15));
		JB_SetRef(JB_str_31, JB_Str3("Unknown class.", 14));
		JB_SetRef(JB_str_30, JB_Str3("µs", 3));
		JB_SetRef(JB_str_29, JB_Str3("          ", 10));
		JB_SetRef(JB_str_28, JB_Str3(".", 1));
		JB_SetRef(JB_str_27, JB_Str3(". New Count ", 12));
		JB_SetRef(JB_str_26, JB_Str3(" bytes in ", 10));
		JB_SetRef(JB_str_25, JB_Str3("Leaked ", 7));
		JB_SetRef(JB_str_24, JB_Str3(", trying to ", 12));
		JB_SetRef(JB_str_23, JB_Str3("'", 1));
		JB_SetRef(JB_str_22, JB_Str3("Can’t find '", 14));
		JB_SetRef(JB_str_21, JB_Str3("/", 1));
		JB_SetRef(JB_str_20, JB_Str3("~/Documents/", 12));
		...
		JB_SyxArg = JB_Syntax__StdNew(JB_Msg_Arg__, JB_str_257, JB_str_258);
		JB_SyxAcc = JB_Syntax__StdNew(JB_Msg_Acc__, JB_str_259, JB_str_260);
		JB_SyxAdj = JB_Syntax__StdNew(JB_Msg_Adj__, JB_str_261, JB_str_262);
		JB_SyxARel = JB_Syntax__StdNew(JB_Msg_ARel__, JB_str_263, JB_str_264);
		JB_SyxArr = JB_Syntax__StdNew(JB_Msg_Arr__, JB_str_265, JB_str_266);
		JB_SyxBack = JB_Syntax__StdNew(JB_Msg_Back__, JB_str_267, JB_str_268);
		JB_SyxSStr = JB_Syntax__StdNew(JB_Msg_SStr__, JB_str_269, JB_str_270);
		JB_SyxBra = JB_Syntax__StdNew(JB_Msg_Bra__, JB_str_271, JB_str_272);
		JB_SyxBRel = JB_Syntax__StdNew(JB_Msg_BRel__, JB_str_273, JB_str_274);
		JB_SyxChar = JB_Syntax__StdNew(JB_Msg_Char__, JB_str_189, JB_str_275);
		JB_SyxDecl = JB_Syntax__StdNew(JB_Msg_Decl__, JB_str_276, JB_str_277);
		JB_SyxDot = JB_Syntax__StdNew(JB_Msg_Dot__, JB_str_278, JB_str_279);
		JB_SyxEmb = JB_Syntax__StdNew(JB_Msg_Emb__, JB_str_280, JB_str_281);
		JB_SyxERel = JB_Syntax__StdNew(JB_Msg_ERel__, JB_str_282, JB_str_283);
		JB_SyxFunc = JB_Syntax__StdNew(JB_Msg_Func__, JB_str_284, JB_str_285);
		JB_SyxItem = JB_Syntax__StdNew(JB_Msg_Item__, JB_str_286, JB_str_0);
		JB_SyxList = JB_Syntax__StdNew(JB_Msg_List__, JB_str_177, JB_str_0);
		JB_SyxName = JB_Syntax__StdNew(JB_Msg_Name__, JB_str_287, JB_str_0);
		JB_SyxNum = JB_Syntax__StdNew(JB_Msg_Num__, JB_str_288, JB_str_289);
		JB_SyxUnit = JB_Syntax__StdNew(JB_Msg_Unit__, JB_str_290, JB_str_0);
		JB_SyxOat = JB_Syntax__StdNew(JB_Msg_Oat__, JB_str_291, JB_str_292);
		JB_SyxOpp = JB_Syntax__StdNew(JB_Msg_Opp__, JB_str_293, JB_str_294);
		JB_SyxSOpp = JB_Syntax__StdNew(JB_Msg_SOpp__, JB_str_295, JB_str_296);
		JB_SyxRel = JB_Syntax__StdNew(JB_Msg_Rel__, JB_str_297, JB_str_298);
		JB_SyxTRel = JB_Syntax__StdNew(JB_Msg_TRel__, JB_str_299, JB_str_300);
		JB_SyxSDot = JB_Syntax__StdNew(JB_Msg_SDot__, JB_str_301, JB_str_302);
		JB_SyxSCom = JB_Syntax__StdNew(JB_Msg_SCom__, JB_str_303, JB_str_304);
		JB_SyxSheb = JB_Syntax__StdNew(JB_Msg_Sheb__, JB_str_305, JB_str_306);
		JB_SyxStr = JB_Syntax__StdNew(JB_Msg_Str__, JB_str_307, JB_str_308);
		JB_SyxSThg = JB_Syntax__StdNew(JB_Msg_SThg__, JB_str_309, JB_str_310);
		JB_SyxThg = JB_Syntax__StdNew(JB_Msg_Thg__, JB_str_311, JB_str_312);
		JB_SyxTmp = JB_Syntax__StdNew(JB_Msg_Tmp__, JB_str_313, JB_str_314);
		JB_SyxAna = JB_Syntax__StdNew(JB_Msg_Ana__, JB_str_315, JB_str_316);
		JB_SyxSCnj = JB_Syntax__StdNew(JB_Msg_SCnj__, JB_str_317, JB_str_318);
		JB_SyxCnj = JB_Syntax__StdNew(JB_Msg_Cnj__, JB_str_319, JB_str_320);
		JB_SyxType = JB_Syntax__StdNew(JB_Msg_Type__, JB_str_321, JB_str_322);
		JB_SyxYoda = JB_Syntax__StdNew(JB_Msg_Yoda__, JB_str_323, JB_str_324);
		JB_SyxYopp = JB_Syntax__StdNew(JB_Msg_Yopp__, JB_str_325, JB_str_0);
		JB_SyxAsk = JB_Syntax__StdNew(JB_Msg_Ask__, JB_str_326, JB_str_327);
		JB_SyxMsg = JB_Syntax__StdNew(JB_Msg_Msg__, JB_str_328, JB_str_329);
		JB_SyxBnch = JB_Syntax__StdNew(JB_Msg_Bnch__, JB_str_330, JB_str_331);
		JB_SyxTodo = JB_Syntax__StdNew(JB_Msg_Todo__, JB_str_332, JB_str_333);
		JB_SyxBin = JB_Syntax__StdNew(JB_Msg_Bin__, JB_str_334, JB_str_335);
	};
	//// ErrorColors;
...
void JB_Saver_Destructor(ObjectSaver* self) {
	if (self->CantSaveThis) {
		JB_Object_Fail(self->CantSaveThis, JB_str_43);
	}
	JB_Decr(self->Dest);
}
...
JB_String* JB_Wrap_Render(DTWrap* self, FastString* fs_in) {
	...
	if (JB_DataTypeCode_IsFloat(self->DataType)) {
		...
		JB_FS_AppendString(fs, JB_str_34);
...
void JB_Sav_SaveWrite(Saveable* self, ObjectSaver* Saver) {
	Saver->CantSaveThis = self;
	JB_FS_AppendString(Saver->Dest, JB_str_44);
}
...
Message* JB_Str_ParseAs(JB_String* self, JB_String* name) {
	if ((!JB_Str_Exists(name)) or (JB_Str_SyntaxEquals(name, JB_str_176, true))) {
		return JB_Str_Parse(self, nil);
	} else if (JB_Str_SyntaxEquals(name, JB_str_177, true)) {
		return ({
...
Message* JB_Msg_ExitInvisArg(Message* self, int missing) {
	...
	while (true) {
		if ((!(JB_Msg_InInvisArg(curr)))) {
			return JB_Tk__ErrorAlwaysAdd(JB_str_184, self->Position);
		}
...
bool JB_Msg_InInvisArg(Message* self) {
	...
	return (JB_Msg_SyntaxEqualsWithSyntaxBool(p, JB_SyxArg, false)) and (JB_Msg_SyntaxEqualsWithStrBool(p, JB_str_17, false));
}
...
JB_String* JB_Msg_Locate(Message* self) {
	JB_String* fname = JB_Incr(JB_str_172);
	if ((!self)) {
		JB_SafeDecr(fname);
		return fname;
	}
	...
	JB_String* rs = JB_Incr(JB_str_172);
...
int JB_Msg_PrevIndentCheck(Message* self) {
	...
	if ((!((prin != chin) and (prin != ((u8)-1))))) {
		return nil;
	}
	int IndExtra = chin - prin;
	if (JB_int_OperatorIsMul(IndExtra, 2)) {
		if ((IndExtra > 0) and ((JB_Msg_SyntaxEqualsWithSyntaxBool(pr, JB_SyxTmp, false)))) {
			return IndExtra;
		} else if ((IndExtra < 0) and (JB_Msg_InInvisArg(self))) {
			return IndExtra;
		}
	}
	JB_FreeIfDead(JB_Tk__ErrorAlwaysAdd(JB_str_154, self->Position));
	JB_FreeIfDead(JB_Tk__ErrorAlwaysAdd(JB_str_154, pr->Position));
	return 0;
}
...
// -2991435863749934684 -7813277751114326161